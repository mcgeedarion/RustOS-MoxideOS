/* tests/shared/sched_rr_fairness.c
 *
 * Stress: SCHED_RR time-slice fairness.
 */
#define _GNU_SOURCE
#include <sched.h>
#include <pthread.h>
...
int main(void) {
    ...
    for (int i = 0; i < N_THREADS; i++)
        pthread_create(&threads[i], &attr, spinner, (void *)(long)i);
    ...
    if (total == 0) TEST_SKIP("zero iterations — scheduling not available");
