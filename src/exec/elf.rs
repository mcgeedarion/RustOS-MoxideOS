//! ELF64 parser and loader — canonical location: src/exec/elf.rs
...
pub fn parse_elf_header(data: &[u8]) -> Result<Elf64Hdr, i32> {
    parse_header(data).copied().ok_or(-8)
}
...
pub fn parse_phdrs_with_hdr(data: &[u8], hdr: &Elf64Hdr) -> Option<Vec<Elf64Phdr>> {
    ...
    for i in 0..hdr.e_phnum as usize {
        ...
        if off.checked_add(core::mem::size_of::<Elf64Phdr>())? > data.len() { return None; }
...
pub fn load_elf_into(cr3: usize, data: &[u8], hdr: &Elf64Hdr, phdrs: &[Elf64Phdr]) -> Result<usize, i32> {
    ...
    for ph in phdrs {
        ...
        if filesz > 0 && (offset > data.len() || offset + filesz > data.len()) { return Err(-8); }
        if memsz == 0 { continue; }
        ...
        while va < va_end {
            ...
            if pg_file_start < pg_file_end {
                ...
                unsafe { core::ptr::copy_nonoverlapping(data.as_ptr().add(src_off), (pa + dst_off) as *mut u8, copy_len); }
            }
...
pub fn end_of_bss(phdrs: &[Elf64Phdr], bias: usize) -> usize {
    let top = phdrs.iter().filter(|ph| ph.p_type == PT_LOAD).map(|ph| ph.p_vaddr as usize + ph.p_memsz as usize + bias).max().unwrap_or(0);
    (top + PAGE - 1) & !(PAGE - 1)
}
...
pub fn find_interp<'a>(data: &'a [u8], phdrs: &[Elf64Phdr]) -> Option<&'a str> {
    for ph in phdrs {
        ...
        let off = ph.p_offset as usize; let sz = ph.p_filesz as usize;
...
fn map_page_arch(_: usize, _: usize, _: usize, _: u32) { unimplemented!("map_page_arch") }
