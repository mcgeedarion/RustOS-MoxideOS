...
use super::breakpoints::{SwBreakpointTable, HwBreakpointTable, WatchpointTable};
...
//   eflags=18  rsp=19  ss=20  fs_base=21  gs_base=22  ds=23  es=24  fs=25  gs=26
...
fn build_gdb_regs(ureg: &[u64; UREG_COUNT]) -> [u64; X86_REG_COUNT] {
    ...
    for (gdb_idx, &ureg_idx) in GDB_TO_UREG.iter().enumerate() {
        out[gdb_idx] = if ureg_idx < UREG_COUNT { ureg[ureg_idx] } else { 0 };
    }
    out
}
