...
//!
//! This file retains:
//! - `GDB_TO_UREG` / `build_gdb_regs` / `unpack_gdb_regs` (ptrace path; maps `user_regs_struct`
//!   indices — not `TrapFrame` — so out of scope for `GdbArch`).
...
use super::arch::{decode_hex_bytes, encode_hex_bytes, parse_hex_u64, GdbArch, X86_64};
...
const GDB_TO_UREG: [usize; X86_REG_COUNT] = [
    10, 5, 11, 12, 13, 14, 4, 19, 9, 8, 7, 6, 3, 2, 1, 0, 16, 18, 17, 20, 23, 24, 25, 26,
];
