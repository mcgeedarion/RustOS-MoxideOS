...
impl GdbTarget {
    ...
    pub fn attach(pid: usize) -> Option<Self> {
        ...
        if mem_fd < 0 || regs_fd < 0 || ctl_fd < 0 {
            ...
            if mem_fd  >= 0 { proc_fd_close(debugger_pid, mem_fd  as usize); }
            if regs_fd >= 0 { proc_fd_close(debugger_pid, regs_fd as usize); }
            if ctl_fd  >= 0 { proc_fd_close(debugger_pid, ctl_fd  as usize); }
            return None;
        }
    ...
    pub fn read_mem(&self, vaddr: u64, len: usize) -> Vec<u8> {
        ...
        let bfd = crate::fs::process_fd::proc_fd_backing(
            self.debugger_pid, self.mem_fd);
        if bfd < 0 { return alloc::vec![0u8; len]; }
        let bfd = bfd as usize;
        if !is_proc_debug_fd(bfd) { return alloc::vec![0u8; len]; }
    ...
    pub fn write_mem(&self, vaddr: u64, data: &[u8]) -> usize {
        ...
        if bfd < 0 { return 0; }
        let bfd = bfd as usize;
        if !is_proc_debug_fd(bfd) { return 0; }
    ...
    pub fn read_regs(&self) -> [u64; UREG_COUNT] {
        use crate::fs::proc_debug::{is_proc_debug_fd, proc_debug_read};
        let bfd = crate::fs::process_fd::proc_fd_backing(
            self.debugger_pid, self.regs_fd);
        let mut regs = [0u64; UREG_COUNT];
        if bfd < 0 { return regs; }
        let bfd = bfd as usize;
        if !is_proc_debug_fd(bfd) { return regs; }
    ...
    pub fn poll_status(&self) -> String {
        ...
        let bfd = crate::fs::process_fd::proc_fd_backing(
            self.debugger_pid, self.ctl_fd);
        if bfd < 0 { return String::from("none"); }
        let bfd = bfd as usize;
        if !is_proc_debug_fd(bfd) { return String::from("none"); }
        let mut buf = [0u8; 16];
        let n = proc_debug_read(bfd, &mut buf, 0);
        if n <= 0 { return String::from("none"); }
