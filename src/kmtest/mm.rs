...
fn mm_map_unmap() -> KmTestResult {
    let addr = sys_mmap(0, PAGE_SIZE, PROT_READ | PROT_WRITE,
                        MAP_ANON | MAP_PRIVATE, usize::MAX, 0);
...
/// mmap two pages, munmap only the first, verify the second is still accessible.
fn mm_partial_unmap() -> KmTestResult {
    let addr = sys_mmap(0, PAGE_SIZE * 2, PROT_READ | PROT_WRITE,
                        MAP_ANON | MAP_PRIVATE, usize::MAX, 0);
    if addr <= 0 { return Err("mmap (partial-unmap) failed"); }
    let base = addr as usize;
    // Unmap only first page.
    let r = sys_munmap(base, PAGE_SIZE);
    if r != 0 { return Err("munmap first page failed"); }
    ...
    if v != 0xCD { return Err("second page lost after partial unmap"); }
...
fn mm_mprotect_upgrade() -> KmTestResult {
    let addr = sys_mmap(0, PAGE_SIZE, PROT_READ,
                        MAP_ANON | MAP_PRIVATE, usize::MAX, 0);
    if addr <= 0 { return Err("mmap (mprotect) failed"); }
    let base = addr as usize;
    ...
    let _ = sys_munmap(base, PAGE_SIZE);
    if v != 0x55 { return Err("write after mprotect upgrade returned wrong value"); }
    Ok(())
}
...
fn mm_mprotect_none_roundtrip() -> KmTestResult {
    let addr = sys_mmap(0, PAGE_SIZE, PROT_READ | PROT_WRITE,
                        MAP_ANON | MAP_PRIVATE, usize::MAX, 0);
    if addr <= 0 { return Err("mmap (prot_none) failed"); }
    let base = addr as usize;
    // Remove all permissions.
    let r1 = sys_mprotect(base, PAGE_SIZE, PROT_NONE);
    if r1 != 0 { let _ = sys_munmap(base, PAGE_SIZE); return Err("mprotect PROT_NONE failed"); }
    // Restore.
    let r2 = sys_mprotect(base, PAGE_SIZE, PROT_READ | PROT_WRITE);
    if r2 != 0 { let _ = sys_munmap(base, PAGE_SIZE); return Err("mprotect restore from NONE failed"); }
    let p = base as *mut u8;
...
fn mm_double_unmap_safe() -> KmTestResult {
    let addr = sys_mmap(0, PAGE_SIZE, PROT_READ | PROT_WRITE,
                        MAP_ANON | MAP_PRIVATE, usize::MAX, 0);
    if addr <= 0 { return Err("mmap (double-unmap) failed"); }
    let base = addr as usize;
    let r1 = sys_munmap(base, PAGE_SIZE);
    if r1 != 0 { return Err("first munmap failed"); }
...
fn mm_large_alloc_graceful() -> KmTestResult {
    ...
    let addr = sys_mmap(0, LARGE, PROT_READ | PROT_WRITE,
                        MAP_ANON | MAP_PRIVATE, usize::MAX, 0);
...
/// regions and verifies write isolation — a lightweight proxy for COW semantics.
fn mm_write_isolation() -> KmTestResult {
    let a = sys_mmap(0, PAGE_SIZE, PROT_READ | PROT_WRITE,
                     MAP_ANON | MAP_PRIVATE, usize::MAX, 0);
    let b = sys_mmap(0, PAGE_SIZE, PROT_READ | PROT_WRITE,
                     MAP_ANON | MAP_PRIVATE, usize::MAX, 0);
    if a <= 0 || b <= 0 { return Err("mmap (write-isolation) failed"); }
    ...
    if va != 0xAA { return Err("page A corrupted by page B write"); }
    if vb != 0xBB { return Err("page B value incorrect"); }
    Ok(())
}
