...
use crate::fs::{
    io_syscalls::{sys_open, sys_read, sys_write, sys_close},
    stat_syscalls::{sys_stat, sys_unlink, sys_rename},
...
fn path(p: &[u8]) -> usize { p.as_ptr() as usize }
...
fn fs_write_read_roundtrip() -> KmTestResult {
    ...
    if fd < 0 { return Err("open for write failed"); }
    let n = sys_write(fd as usize, data.as_ptr() as usize, data.len());
    if n != data.len() as isize { sys_close(fd as usize); return Err("write returned wrong count"); }
    sys_close(fd as usize);

    let fd2 = sys_open(path(PATH_A), O_RDONLY, 0);
    if fd2 < 0 { return Err("open for read failed"); }
    let mut buf = [0u8; 32];
    let r = sys_read(fd2 as usize, buf.as_mut_ptr() as usize, buf.len());
    sys_close(fd2 as usize);
    if r != data.len() as isize { return Err("read returned wrong count"); }
    if &buf[..data.len()] != data { return Err("read data does not match written data"); }
...
fn fs_excl_existing() -> KmTestResult {
    // Ensure file exists.
    let fd = sys_open(path(PATH_A), O_WRONLY | O_CREAT | O_TRUNC, S_MODE);
    if fd < 0 { return Err("setup open failed"); }
    sys_close(fd as usize);
    ...
    // -EEXIST = -17
    if fd2 != -17 { return Err("O_EXCL returned wrong errno (expected EEXIST)"); }
    Ok(())
}
...
fn fs_rename_atomic() -> KmTestResult {
    ...
    if fd_dst < 0 { return Err("destination missing after rename"); }
    let mut buf = [0u8; 32];
...
fn fs_unlink_removes() -> KmTestResult {
    let fd = sys_open(path(PATH_A), O_WRONLY | O_CREAT | O_TRUNC, S_MODE);
    if fd < 0 { return Err("unlink setup failed"); }
    sys_close(fd as usize);

    let r = sys_unlink(path(PATH_A));
    if r != 0 { return Err("unlink failed"); }
...
fn fs_stat_size() -> KmTestResult {
    ...
    let fd = sys_open(path(PATH_A), O_WRONLY | O_CREAT | O_TRUNC, S_MODE);
    if fd < 0 { return Err("stat size open failed"); }
    sys_write(fd as usize, data.as_ptr() as usize, data.len());
    sys_close(fd as usize);
