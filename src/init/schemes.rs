...
//! (blk, tty) come before higher-level ones (net, tcp) so that log messages
//! appear in dependency order.
//!
//! # Boot integration
//!
//! ```rust
//! // In kernel_main.rs, after drivers::nic::init() and before pid-1 spawn:
...
pub fn init() {
    ...
    // Registered early so inner schemes that open config files (e.g. /etc/resolv.conf)
    // can do so via open_url before network init.
