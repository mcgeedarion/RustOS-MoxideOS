...
// ── NR 39  getpid ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
fn sys_getpid_impl() -> isize {
    crate::proc::scheduler::current_pid() as isize
}
fn sys_getppid_impl() -> isize {
    crate::proc::scheduler::current_ppid() as isize
}
fn sys_gettid_impl() -> isize {
    crate::proc::scheduler::current_tid() as isize
}

// ── NR 102/104/107/108  getuid/getgid/geteuid/getegid ──────────────────────────────────────────────
fn sys_getuid_impl() -> isize {
    0
}
fn sys_getgid_impl() -> isize {
    0
}
fn sys_geteuid_impl() -> isize {
    0
}
fn sys_getegid_impl() -> isize {
    0
}

// ── NR 63  uname ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
fn sys_uname_impl(buf: usize) -> isize {
    if buf == 0 || !validate_user_ptr(buf, 390) {
        return -14;
    }
    ...
    if copy_to_user(buf, &dst).is_err() {
        return -14;
    }
    0
}
...
fn sys_times_impl(buf_va: usize) -> isize {
    ...
    let (utime_ns, stime_ns) =
        crate::proc::scheduler::with_proc(pid, |p| (p.utime_ns, p.stime_ns)).unwrap_or((0, 0));
    ...
    if buf_va != 0 {
        ...
        if copy_to_user(buf_va, &buf).is_err() {
            return -14;
        }
    }
...
fn sys_personality_impl(persona: u32) -> isize {
    ...
    if persona == GET_PERSONA {
        ...
        return crate::proc::scheduler::with_proc(pid, |p| p.personality).unwrap_or(0) as isize;
    }
...
fn sys_prctl_impl(op: i32, a2: usize, _a3: usize, _a4: usize, _a5: usize) -> isize {
    ...
    match op {
        PR_SET_NAME => {
            let mut name = [0u8; 16];
            if copy_from_user(&mut name[..15], a2).is_err() {
                return -14;
            }
        ...
        PR_GET_NAME => {
            ...
            if copy_to_user(a2, &name).is_err() {
                return -14;
            }
            0
        }
...
fn sys_time_impl(t_va: usize) -> isize {
    ...
    if t_va != 0 {
        if copy_to_user(t_va, &secs.to_le_bytes()).is_err() {
            return -14;
        }
    }
    secs as isize
...
fn sys_clock_getres_impl(_clkid: u32, res_va: usize) -> isize {
    ...
    if res_va != 0 {
        ...
        if copy_to_user(res_va, &ts).is_err() {
            return -14;
        }
    }
    0
...
pub(super) fn sys_futimesat_impl(dirfd: i32, path_va: usize, times_va: usize) -> isize {
    ...
    let mut tv = [0u8; 32];
    if crate::uaccess::copy_from_user(&mut tv, times_va).is_err() {
        return -14;
    }
