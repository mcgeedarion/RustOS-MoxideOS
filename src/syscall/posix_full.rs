...
pub(super) fn sys_getuid_impl() -> isize {
    crate::proc::creds::sys_getuid()
}
pub(super) fn sys_getgid_impl() -> isize {
    crate::proc::creds::sys_getgid()
}
pub(super) fn sys_geteuid_impl() -> isize {
    crate::proc::creds::sys_geteuid()
}
pub(super) fn sys_getegid_impl() -> isize {
    crate::proc::creds::sys_getegid()
}

pub(super) fn sys_setuid_impl(uid: u32) -> isize {
    crate::proc::creds::sys_setuid(uid)
}
pub(super) fn sys_setgid_impl(gid: u32) -> isize {
    crate::proc::creds::sys_setgid(gid)
}

pub(super) fn sys_setreuid_impl(_ruid: u32, _euid: u32) -> isize {
    0
}
pub(super) fn sys_setregid_impl(_rgid: u32, _egid: u32) -> isize {
    0
}
...
pub(super) fn sys_setgroups_impl(_size: i32, _list_va: usize) -> isize {
    0
}

pub(super) fn sys_setresuid_impl(_ruid: u32, _euid: u32, _suid: u32) -> isize {
    0
}
pub(super) fn sys_setresgid_impl(_rgid: u32, _egid: u32, _sgid: u32) -> isize {
    0
}
...
pub(super) fn sys_timer_create_impl(clockid: i32, sigevent_va: usize, timerid_va: usize) -> isize {
...
pub(super) fn sys_gettimeofday_impl(tv_va: usize, _tz_va: usize) -> isize {
    ...
    if copy_to_user(tv_va, &buf).is_err() {
        return -14;
    }
    0
}
...
pub(super) fn sys_time_impl(tloc_va: usize) -> isize {
    ...
    if tloc_va == 0 {
        return now_sec as isize;
    }
    if copy_to_user(tloc_va, &now_sec.to_le_bytes()).is_err() {
        return -14;
    }
    now_sec as isize
}
...
pub(super) fn sys_adjtimex_impl(_buf_va: usize) -> isize {
    -1
}
...
pub(super) fn sys_prlimit64_impl(pid: u32, resource: u32, new_va: usize, old_va: usize) -> isize {
...
pub(super) fn sys_acct_impl(_path_va: usize) -> isize {
    -38
}
...
/// NR 166  umount2
pub(super) fn sys_umount2_impl(_tgt: usize, _flags: i32) -> isize {
    0
}
...
pub(super) fn sys_sethostname_impl(name_va: usize, len: usize) -> isize {
    let l = len.min(64);
    let mut buf = [0u8; 64];
    if copy_from_user(&mut buf[..l], name_va).is_err() {
        return -14;
    }
...
pub(super) fn sys_copy_file_range_impl(
    ...
    if off_in_va != 0 {
        let mut buf = [0u8; 8];
        if copy_from_user(&mut buf, off_in_va).is_err() {
            return -14;
        }
    ...
    if off_out_va != 0 {
        let mut buf = [0u8; 8];
        if copy_from_user(&mut buf, off_out_va).is_err() {
            return -14;
        }
    ...
    if off_in_va != 0 {
        ...
        if copy_to_user(off_in_va, &new_off.to_le_bytes()).is_err() {
            return -14;
        }
    }
    if off_out_va != 0 {
        let new_off = crate::fs::vfs::seek(fd_out, 0, crate::fs::vfs::SEEK_CUR) as i64;
        if copy_to_user(off_out_va, &new_off.to_le_bytes()).is_err() {
            return -14;
        }
    }
    written
...
pub(super) fn sys_pwritev2_impl(
    fd: usize,
    iov_va: usize,
    iovcnt: usize,
    pos_l: usize,
    ...
    for i in 0..iovcnt {
        ...
        if copy_from_user(&mut iov_buf, iov_va + i * 16).is_err() {
            return -14;
        }
        ...
        let n = crate::syscall::sys_pwrite64_impl(
            fd,
            base,
            len,
            if offset >= 0 {
...
pub(super) fn sys_statx_impl(
    ...
    if copy_to_user(statxbuf_va, &sx).is_err() {
        return -14;
    }
    0
}
...
pub(super) fn sys_pkey_alloc_impl(_flags: u32, _access_rights: u64) -> isize {
    0
}

/// NR 331  pkey_free — accept.
pub(super) fn sys_pkey_free_impl(_pkey: i32) -> isize {
    0
}
...
pub(super) fn sys_mlock2_impl(_addr: usize, _len: usize, _flags: u32) -> isize {
    0
}
...
pub(super) fn sys_sched_getattr_impl(_pid: usize, attr_va: usize, size: u32, _flags: u32) -> isize {
    ...
    if copy_to_user(attr_va, &buf).is_err() {
        return -14;
    }
    0
}
...
pub(super) fn sys_eperm_impl() -> isize {
    -1
}
...
pub(super) fn sys_process_vm_readv_impl(
    ...
    'outer: for ri in 0..riovcnt {
        let mut riov = [0u8; 16];
        if copy_from_user(&mut riov, rvec_va + ri * 16).is_err() {
            break;
        }
        ...
        while remaining > 0 {
            if local_iov_idx >= liovcnt {
                break 'outer;
            }

            let mut liov = [0u8; 16];
            if copy_from_user(&mut liov, lvec_va + local_iov_idx * 16).is_err() {
                break 'outer;
            }
            let local_base = usize::from_le_bytes(liov[0..8].try_into().unwrap());
            let local_len = usize::from_le_bytes(liov[8..16].try_into().unwrap());
            let local_avail = if local_len > local_off {
                local_len - local_off
            } else {
                local_iov_idx += 1;
                local_off = 0;
...
pub(super) fn sys_process_vm_writev_impl(
    ...
    'outer: for ri in 0..riovcnt {
        ...
        while remaining > 0 {
            ...
            unsafe {
                core::ptr::copy_nonoverlapping(bounce.as_ptr(), dst, available);
            }
