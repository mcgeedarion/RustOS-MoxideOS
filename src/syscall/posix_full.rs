...
pub(super) fn sys_getrlimit_impl(resource: u32, rlim_va: usize) -> isize {
    ...
    if crate::mm::uaccess::copy_to_user(rlim_va, &buf).is_err() { return -14; }
    0
}

/// NR 160  setrlimit(resource, rlim_va) — accept silently; we don't enforce.
pub(super) fn sys_setrlimit_impl(_resource: u32, _rlim_va: usize) -> isize { 0 }

/// NR 302  prlimit64(pid, resource, new_va, old_va)
pub(super) fn sys_prlimit64_impl(
    _pid: u32, resource: u32, _new_va: usize, old_va: usize,
) -> isize {
    if old_va != 0 {
...
pub(super) fn sys_sysinfo_impl(info_va: usize) -> isize {
    ...
    if crate::mm::uaccess::copy_to_user(info_va, &buf).is_err() { return -14; }
    0
}
...
pub(super) fn sys_times_impl(tbuf_va: usize) -> isize {
    if tbuf_va != 0 {
        ...
        if crate::mm::uaccess::copy_to_user(tbuf_va, &buf).is_err() { return -14; }
    }
...
pub(super) fn sys_prctl_impl(op: i32, a2: usize, _a3: usize, _a4: usize, _a5: usize) -> isize {
    ...
    match op {
        PR_SET_NAME => {
            ...
            if let Ok(name) = crate::mm::uaccess::read_user_str(a2, 16) {
        ...
        PR_GET_NAME => {
            ...
            if crate::mm::uaccess::copy_to_user(a2, &buf).is_err() { return -14; }
            0
        }
        PR_SET_DUMPABLE  => 0,
        PR_GET_DUMPABLE  => 1,
...
pub(super) fn sys_arch_prctl_impl(code: i32, addr: usize) -> isize {
    ...
    match code {
        ARCH_SET_FS => {
            crate::proc::scheduler::with_proc_mut(pid, |p, _| p.fs_base = addr)
                .unwrap_or(());
            unsafe { core::arch::x86_64::__cpuid(0); } // serialise
            crate::arch::x86_64::msr::write_msr(0xC0000100, addr as u64); // IA32_FS_BASE
            0
        }
        ARCH_GET_FS => {
            let base = crate::proc::scheduler::with_proc(pid, |p| p.fs_base)
                           .unwrap_or(0);
            if crate::mm::uaccess::copy_to_user(addr, &base.to_le_bytes()).is_err() { return -14; }
            0
        }
        ARCH_SET_GS => {
            crate::proc::scheduler::with_proc_mut(pid, |p, _| p.gs_base = addr)
                .unwrap_or(());
            crate::arch::x86_64::msr::write_msr(0xC0000101, addr as u64); // IA32_GS_BASE
            0
        }
        ARCH_GET_GS => {
            let base = crate::proc::scheduler::with_proc(pid, |p| p.gs_base)
                           .unwrap_or(0);
            if crate::mm::uaccess::copy_to_user(addr, &base.to_le_bytes()).is_err() { return -14; }
            0
        }
...
pub(super) fn sys_copy_file_range_impl(
    fd_in: usize, off_in_va: usize, fd_out: usize, off_out_va: usize,
...
pub(super) fn sys_capset_impl(_hdr_va: usize, _data_va: usize) -> isize { 0 }
