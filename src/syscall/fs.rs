    ...
    if is_scheme_url(path) {
        match SCHEME_TABLE.open(path, flags) {
            Ok((handler, fid)) => {
                ...
                let fd = proc.fd_table()
                    .alloc(FdEntry::Scheme { handler, fid });
                fd as i64
            }
            Err(e) => e.to_errno(),
        }
...
pub fn sys_read(fd: RawFd, buf: &mut [u8]) -> i64 {
    ...
    match entry {
        FdEntry::Legacy(node) => {
            vfs_ops::read(&node, buf)
                .map(|n| n as i64)
                .unwrap_or_else(|e| e.to_errno())
        }
        FdEntry::Scheme { handler, fid } => {
            handler.read(fid, buf)
                .map(|n| n as i64)
                .unwrap_or_else(|e| e.to_errno())
        }
    }
...
pub fn sys_write(fd: RawFd, buf: &[u8]) -> i64 {
    ...
    match entry {
        FdEntry::Legacy(node) => {
            vfs_ops::write(&node, buf)
                .map(|n| n as i64)
                .unwrap_or_else(|e| e.to_errno())
        }
        FdEntry::Scheme { handler, fid } => {
            handler.write(fid, buf)
                .map(|n| n as i64)
                .unwrap_or_else(|e| e.to_errno())
        }
    }
...
pub fn sys_seek(fd: RawFd, offset: i64, whence: u8) -> i64 {
    ...
    match entry {
        FdEntry::Legacy(node) => {
            vfs_ops::seek(&node, offset, whence)
                .map(|pos| pos as i64)
                .unwrap_or_else(|e| e.to_errno())
        }
        FdEntry::Scheme { handler, fid } => {
            handler.seek(fid, offset, whence)
                .unwrap_or_else(|e| e.to_errno())
        }
    }
}
...
pub fn sys_ioctl(fd: RawFd, cmd: u64, arg: usize) -> i64 {
    ...
    match entry {
        FdEntry::Legacy(node) => {
            vfs_ops::ioctl(&node, cmd, arg)
                .map(|n| n as i64)
                .unwrap_or_else(|e| e.to_errno())
        }
        FdEntry::Scheme { handler, fid } => {
            handler.ioctl(fid, cmd, arg)
                .map(|n| n as i64)
                .unwrap_or_else(|e| e.to_errno())
        }
    }
