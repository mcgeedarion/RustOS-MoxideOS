//! Virtio-net MMIO driver (for RISC-V `virt` machine and other MMIO
//! transports).
//!
...
unsafe fn _init(base: usize) {
    if read32(base, MMIO_MAGIC) != 0x7472_6976 {
        return;
    }
    if read32(base, MMIO_DEVICE_ID) != 1 {
        return;
    } // net device
    ...
    let mut mac = [0u8; 6];
    for i in 0..6 {
        mac[i] = read8(base, MMIO_CONFIG + i);
    }
