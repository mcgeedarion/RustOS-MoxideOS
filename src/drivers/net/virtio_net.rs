...
unsafe fn _init(iobase: u16) {
    ...
    out8(iobase + VIRTIO_PCI_STATUS, STATUS_ACK | STATUS_DRIVER | STATUS_FEAT_OK);

    let rxq = setup_queue(iobase, QUEUE_RX);
    let txq = setup_queue(iobase, QUEUE_TX);

    let mut mac = [0u8; 6];
    for i in 0..6 { mac[i] = in8(iobase + VIRTIO_PCI_MAC + i as u16); }

    out8(iobase + VIRTIO_PCI_STATUS, STATUS_ACK | STATUS_DRIVER | STATUS_FEAT_OK | STATUS_OK);
...
unsafe fn refill_rx(q: &mut Queue, iobase: u16) {
    for i in 0..QSZ as u16 { recycle_rx(q, i, iobase); }
}
...
#[inline]
unsafe fn notify(iobase: u16, q: u16) { out16(iobase + 0x10, q); }
#[inline]
...
#[inline] unsafe fn in8(port: u16) -> u8 { let mut v: u8; core::arch::asm!("in al, dx", out("al") v, in("dx") port); v }
#[inline] unsafe fn in16(port: u16) -> u16 { let mut v: u16; core::arch::asm!("in ax, dx", out("ax") v, in("dx") port); v }
#[inline] unsafe fn in32(port: u16) -> u32 { let mut v: u32; core::arch::asm!("in eax, dx", out("eax") v, in("dx") port); v }
#[inline] unsafe fn out8(port: u16, val: u8) { core::arch::asm!("out dx, al", in("dx") port, in("al") val); }
#[inline] unsafe fn out16(port: u16, val: u16) { core::arch::asm!("out dx, ax", in("dx") port, in("ax") val); }
#[inline] unsafe fn out32(port: u16, val: u32) { core::arch::asm!("out dx, eax", in("dx") port, in("eax") val); }
