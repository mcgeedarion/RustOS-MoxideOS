...
use crate::drivers::pcie::{find_device_by_class, pci_enable_msi_ex, pci_enable_msix};
use crate::mm::pmm;
...
impl Xhci {
    ...
    unsafe fn cap_read32(&self, off: usize) -> u32 {
        r32(self.cap_base + off)
    }
    unsafe fn op_read32(&self, off: usize) -> u32 {
        r32(self.op_base + off)
    }
    unsafe fn op_write32(&self, off: usize, v: u32) {
        w32(self.op_base + off, v)
    }
    unsafe fn rt_write32(&self, off: usize, v: u32) {
        w32(self.rt_base + off, v)
    }
    unsafe fn rt_read32(&self, off: usize) -> u32 {
        r32(self.rt_base + off)
    }
    unsafe fn db_write32(&self, slot: usize, v: u32) {
        w32(self.db_base + slot * 4, v)
    }
...
pub fn xhci_probe() {
    let dev = match find_device_by_class(USB_CLASS) {
        Some(d) => d,
        None => {
            crate::println!("xhci: no xHCI controller found");
            return;
        }
    };
    ...
    unsafe {
        init_controller(bar0 as usize, &dev);
    }
}
...
unsafe fn handle_port_status_change(xhci: &mut Xhci, trb: Trb) {
    ...
    if portsc & PORTSC_CCS != 0 {
        ...
        w32(
            xhci.op_base + portsc_off,
            (portsc | PORTSC_PR) & !(PORTSC_PED),
        );
...
unsafe fn send_enable_slot(xhci: &mut Xhci, port: u8) {
    ...
    let slot_id = match (1..MAX_SLOTS).find(|&i| xhci.slots[i].state == SlotState::Free) {
        Some(i) => i,
        None => {
            crate::println!("xhci: no free slots");
            return;
        }
    };
...
unsafe fn send_address_device(xhci: &mut Xhci, slot_id: usize) {
    ...
    slot_ctx
        .add(0)
        .write_volatile((1 << 27) | ((port as u32) << 16)); // ctx_entries=1, port
    ...
    ep0_ctx
        .add(1)
        .write_volatile((4 << 3) | (3 << 1) | (8 << 16)); // ep_type=4 (ctrl), mps=8
