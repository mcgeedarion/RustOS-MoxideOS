...
#[repr(C, packed)]
#[derive(Clone, Copy)]
struct Avail { flags: u16, idx: u16, ring: [u16; QSZ] }
...
#[repr(C, packed)]
#[derive(Clone, Copy)]
struct Used { flags: u16, idx: u16, ring: [UsedElem; QSZ] }
...
pub fn display_info() -> Option<DisplayInfo> {
    GPU.lock().as_ref().map(|g| DisplayInfo {
        width: g.width, height: g.height, pitch: g.width * 4, bpp: 32,
    })
}
...
pub fn clear(argb: u32) {
    ...
    if let Some(gpu) = g.as_ref() {
        if let Some(fb) = &gpu.fb { fb.clear(argb); }
    }
}
...
pub fn blit(x: u32, y: u32, w: u32, h: u32, pixels: &[u32]) {
    let g = GPU.lock();
    if let Some(gpu) = g.as_ref() {
        if let Some(fb) = &gpu.fb { fb.blit(x, y, w, h, pixels); }
    }
}
...
unsafe fn _init(base: usize) {
    ...
    if read32(base, MMIO_DEVICE_ID) != DEVICE_ID_GPU { return; }
...
unsafe fn get_display_info(base: usize, _vq: &Vq) -> (u32, u32) {
    ...
    *cmd = CtrlHdr { hdr_type: CMD_GET_DISPLAY_INFO, ..Default::default() };
...
unsafe fn create_resource(base: usize, _vq: &Vq, res_id: u32, w: u32, h: u32) {
    ...
    *cmd = CmdResource2d {
        hdr: CtrlHdr { hdr_type: CMD_RESOURCE_CREATE_2D, ..Default::default() },
        resource_id: res_id, format: FORMAT_B8G8R8X8, width: w, height: h,
    };
...
unsafe fn attach_backing(base: usize, _vq: &Vq, res_id: u32, fb_phys: u64, size: u32) {
    ...
    *cmd = CmdAttachBacking {
        hdr: CtrlHdr { hdr_type: CMD_RESOURCE_ATTACH_BACKING, ..Default::default() },
        resource_id: res_id, nr_entries: 1,
        entry: MemEntry { addr: fb_phys, length: size, _pad: 0 },
    };
...
unsafe fn set_scanout(base: usize, _vq: &Vq, res_id: u32, w: u32, h: u32) {
    ...
    *cmd = CmdSetScanout {
        hdr: CtrlHdr { hdr_type: CMD_SET_SCANOUT, ..Default::default() },
        r: Rect { x: 0, y: 0, width: w, height: h },
        scanout_id: 0, resource_id: res_id,
    };
...
unsafe fn transfer_flush(base: usize, _vq: &Vq, res_id: u32, w: u32, h: u32) {
    ...
    *t = CmdTransfer2d {
        hdr: CtrlHdr { hdr_type: CMD_TRANSFER_TO_HOST_2D, ..Default::default() },
        r: Rect { x: 0, y: 0, width: w, height: h },
        offset: 0, resource_id: res_id, _pad: 0,
    };
    ...
    *f = CmdFlush {
        hdr: CtrlHdr { hdr_type: CMD_RESOURCE_FLUSH, ..Default::default() },
        r: Rect { x: 0, y: 0, width: w, height: h },
        resource_id: res_id, _pad: 0,
    };
...
unsafe fn setup_vq(base: usize, q: u32) -> Vq {
    ...
    Vq { desc, avail, used, last_used: 0, free_head: 0 }
}

#[inline] fn align_up(x: usize, a: usize) -> usize { (x + a - 1) & !(a - 1) }
...
#[inline] unsafe fn read32(b: usize, o: usize) -> u32 { read_volatile((b+o) as *const u32) }
#[inline] unsafe fn write32(b: usize, o: usize, v: u32) { write_volatile((b+o) as *mut u32, v); }
