...
pub fn init_lfb(base: u64, width: u32, height: u32, pitch: u32) {
    let fb = Framebuffer { phys: base, width, height, pitch, format: PixelFormat::Xrgb8888 };
    *VGA.lock() = Some(VgaState {
        mode: VgaMode::Lfb(fb),
        col: 0, row: 0,
        attr: color::attr(color::LIGHT_GREY, color::BLACK),
...
pub fn is_initialised() -> bool { VGA.lock().is_some() }

pub fn display_info() -> Option<DisplayInfo> {
    VGA.lock().as_ref().map(|v| match &v.mode {
        VgaMode::Text => DisplayInfo { width: 80, height: 25, pitch: 160, bpp: 16 },
        VgaMode::Lfb(fb) => DisplayInfo {
            width: fb.width, height: fb.height, pitch: fb.pitch, bpp: 32,
        },
    })
}
...
pub fn putchar(c: u8) {
    ...
    match c {
        ...
        b'\r' => { st.col = 0; }
        b'\t' => { st.col = (st.col + 8) & !7; }
        b'\x08' => { if st.col > 0 { st.col -= 1; } }
        _ => {
            text_write_at(st.col, st.row, c, st.attr);
            st.col += 1;
        }
    }
    if st.col >= VGA_TEXT_COLS { st.col = 0; st.row += 1; }
    if st.row >= VGA_TEXT_ROWS { scroll(); st.row = VGA_TEXT_ROWS - 1; }
...
fn clear_text_buffer() {
    for i in 0..VGA_TEXT_CELLS {
        ...
        unsafe {
            ...
            write_volatile((VGA_TEXT_BASE + idx + 1) as *mut u8,
