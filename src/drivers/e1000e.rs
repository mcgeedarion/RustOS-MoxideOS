...
use crate::drivers::pcie::{find_device_by_id, pci_enable_msi_ex, pci_enable_msix};
use crate::mm::pmm;
use spin::Mutex;
...
pub fn e1000e_probe() -> bool {
    let dev = DEVIDS.iter().find_map(|&id| find_device_by_id(VENDOR, id));
...
unsafe fn init(bar0: u64) {
    ...
    let tctl: u32 = TCTL_EN | TCTL_PSP | (0x10 << TCTL_CT_SHIFT) | (0x40 << TCTL_COLD_SHIFT);
...
fn e1000e_send_frame(frame: &[u8]) -> bool {
    ...
    unsafe {
        core::ptr::copy_nonoverlapping(frame.as_ptr(), dev.tx_bufs[tail], frame.len());
    }
    ...
    unsafe {
        write32(dev.bar0, REG_TDT, dev.tx_tail as u32);
    }
    true
}
...
fn e1000e_rx_poll() {
    ...
    loop {
        ...
        dev.rx_tail = next;
        unsafe {
            write32(dev.bar0, REG_RDT, dev.rx_tail as u32);
        }
    }
}
