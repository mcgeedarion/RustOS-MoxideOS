...
impl CharFifo {
    const fn new() -> Self {
        Self { buf: ['\0'; CHAR_BUF], head: 0, tail: 0 }
    }
...
static KBD: Mutex<KbdState> = Mutex::new(KbdState {
    keys:  KeyState::new(),
    mods:  Mods { shift: false, ctrl: false, alt: false, caps: false },
    chars: CharFifo::new(),
    e0:    false,
});
...
unsafe fn ps2_read() -> u8 {
    let v: u8;
    core::arch::asm!("in al, dx", out("al") v, in("dx") PS2_DATA, options(nomem, nostack)); v
}
...
fn process_key(kbd: &mut KbdState, code: u16, value: i32) {
    ...
    // Update bitmask.
    if down { kbd.keys.set(code); } else { kbd.keys.clear(code); }
...
fn update_mods(m: &mut Mods, code: u16, down: bool) {
    ...
    if code == ls || code == rs { m.shift = down; }
    if code == lc || code == rc { m.ctrl  = down; }
    if code == la || code == ra { m.alt   = down; }
    // Capslock toggles on key-down.
    if code == cl && down { m.caps = !m.caps; }
}
...
pub fn shift_held() -> bool { KBD.lock().mods.shift }
/// Returns true if any ctrl key is held.
pub fn ctrl_held()  -> bool { KBD.lock().mods.ctrl  }
/// Returns true if any alt key is held.
pub fn alt_held()   -> bool { KBD.lock().mods.alt   }
