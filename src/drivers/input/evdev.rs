...
impl EventRing {
    const fn new() -> Self {
        Self {
            buf: [InputEvent { time_sec: 0, time_usec: 0, r#type: 0, code: 0, value: 0 }; RING_CAP],
            head: 0,
            tail: 0,
        }
    }
    fn push(&mut self, ev: InputEvent) {
        ...
        if next == self.head { return; } // drop on overflow
...
pub fn sync() {
    push(InputEvent { r#type: EV_SYN, code: SYN_REPORT, value: 0, ..Default::default() });
}
...
fn timestamp() -> (u32, u32) {
    ...
    ((ns / 1_000_000_000) as u32, ((ns % 1_000_000_000) / 1_000) as u32)
}

/// Convenience: build and push a key event.
pub fn push_key(code: u16, value: i32) {
    let (s, us) = timestamp();
    push(InputEvent { time_sec: s, time_usec: us, r#type: EV_KEY, code, value });
}

/// Convenience: build and push a relative motion event.
pub fn push_rel(code: u16, value: i32) {
    let (s, us) = timestamp();
    push(InputEvent { time_sec: s, time_usec: us, r#type: EV_REL, code, value });
}
