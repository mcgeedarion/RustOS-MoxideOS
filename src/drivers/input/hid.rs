...
pub fn parse_descriptor(desc: &[u8]) -> HidReport {
    ...
    while i < desc.len() {
        let b = desc[i];
        let tag  = b & 0xFC;
        let size = match b & 0x03 { 1 => 1, 2 => 2, 3 => 4, _ => 0 };
        i += 1;
        let val: u32 = match size {
            1 => { let v = desc.get(i).copied().unwrap_or(0) as u32; i += 1; v }
            2 => {
        ...
        match tag {
            TAG_USAGE_PAGE    => { usage_page = val as u16; }
            TAG_USAGE         => { usages.push(val as u16); }
            TAG_LOG_MIN       => { log_min = val as i32; }
            TAG_LOG_MAX       => { log_max = val as i32; }
            TAG_REPORT_SIZE   => { report_size = val; }
            TAG_REPORT_COUNT  => { report_count = val; }
            ...
            TAG_INPUT         => {
                ...
                if flags & INP_CONST == 0 {
                    ...
                    if f.usage_page == UP_KEYBOARD { is_keyboard = true; }
                    fields.push(f);
                }
                usages.clear();
            }
...
pub fn dispatch(report: &HidReport, data: &[u8]) {
    ...
    for field in &report.fields {
        for c in 0..field.report_count as usize {
            ...
            let usage = field.usages.get(c).copied()
                .or_else(|| field.usages.last().copied())
                .unwrap_or(0);

            if field.flags & INP_CONST != 0 { continue; }

            if field.usage_page == UP_GENERIC_DESKTOP {
                if field.flags & INP_REL != 0 {
                    let signed = sign_extend(raw, field.report_size as usize, field.logical_min, field.logical_max);
                    match usage {
