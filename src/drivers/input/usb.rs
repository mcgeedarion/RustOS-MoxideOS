...
unsafe fn _init(mmio: usize) {
    ...
    while read32(opr, OPR_USBSTS) & STS_CNR != 0 { core::hint::spin_loop(); }
    ...
    *erst.add(1) = TR_SIZE as u64;    // size
    ...
    while read32(opr, OPR_USBSTS) & STS_HCH != 0 { core::hint::spin_loop(); }

    let mut dev_ctx: Vec<*mut DevCtx> = Vec::with_capacity(max_slots + 1);
    for _ in 0..=max_slots { dev_ctx.push(core::ptr::null_mut()); }

    *CTRL.lock() = Some(XhciCtrl {
        mmio, opr, db_base, rts_base,
        max_slots,
        cmd_ring, cmd_cycle: 1, cmd_enq: 0,
        evt_ring, evt_cycle: 1, evt_deq: 0,
        dcbaa, dev_ctx,
        int_rings: Vec::new(),
    });
...
unsafe fn attach_device(port: usize) {
    ...
    let mut g = CTRL.lock();
    let c = match g.as_mut() { Some(c) => c, None => return };
...
unsafe fn _poll() {
    ...
    loop {
        ...
        if (trb.ctrl & 1) as u32 != c.evt_cycle as u32 { break; }
