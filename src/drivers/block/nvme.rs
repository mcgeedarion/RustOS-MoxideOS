...
use crate::drivers::platform::pcie::{
...
impl NvmeCtrl {
    ...
    unsafe fn identify_controller(&mut self) -> String {
        ...
        let sqe = Sqe { cdw0: ADMIN_IDENTIFY as u32, nsid: 0, prp1: buf_pa, prp2: 0, cdw10: CNS_CONTROLLER as u32, ..Default::default() };
    ...
    unsafe fn identify_namespace(&mut self) {
        let buf_pa = Self::alloc_page();
        let sqe = Sqe { cdw0: ADMIN_IDENTIFY as u32, nsid: 1, prp1: buf_pa, prp2: 0, cdw10: CNS_NAMESPACE as u32, ..Default::default() };
        let status = self.admin_cmd(sqe);
        if status == 0 {
            let ptr = buf_pa as *const u8;
    ...
    unsafe fn create_io_cq(&mut self, cq_pa: u64) -> bool {
        ...
        let cdw11: u32 = 1 | (1 << 1);
        let sqe = Sqe { cdw0: ADMIN_CREATE_IO_CQ as u32, prp1: cq_pa, cdw10, cdw11, ..Default::default() };
    ...
    unsafe fn create_io_sq(&mut self, sq_pa: u64) -> bool {
        ...
        let sqe = Sqe { cdw0: ADMIN_CREATE_IO_SQ as u32, prp1: sq_pa, cdw10, cdw11, ..Default::default() };
        self.admin_cmd(sqe) == 0
    }
...
pub fn nvme_probe() {
    let dev = match find_device_by_class(PCI_CLASS_STORAGE_NVME) {
        Some(d) => d,
        None => { crate::log::kprintln!("nvme: no NVMe controller found"); return; }
    };
    let bar0 = match dev.bar_mmio(0) {
        Some(b) => b,
        None => { crate::log::kprintln!("nvme: BAR0 not MMIO"); return; }
    };
    dev.enable();
    let irq_mode = if pci_enable_msix(&dev, 0, NVME_IRQ_VECTOR, 0) { "MSI-X" }
        else if pci_enable_msi_ex(&dev, 0, NVME_IRQ_VECTOR) { "MSI" }
        else { "polled" };
    ...
    crate::log::kprintln!("nvme: BAR0={:#x} vs={}.{} irq={}", bar0, (vs >> 16), (vs >> 8) & 0xFF, irq_mode);
    if !unsafe { ctrl.reset() } { crate::log::kprintln!("nvme: reset timeout"); return; }
    unsafe { ctrl.setup_admin_queues(); }
    if !unsafe { ctrl.enable() } { crate::log::kprintln!("nvme: enable timeout / CFS"); return; }
    let model = unsafe { ctrl.identify_controller() };
    crate::log::kprintln!("nvme: model: {}", model);
    unsafe { ctrl.identify_namespace(); }
    crate::log::kprintln!("nvme: ns1 size={} LBAs  lba_shift={}", ctrl.ns_size, ctrl.lba_shift);
    if !unsafe { ctrl.setup_io_queues() } { crate::log::kprintln!("nvme: IO queue creation failed"); return; }
    ctrl.ready = true;
    crate::log::kprintln!("nvme: ready");
}

pub fn nvme_capacity() -> u64 { CTRL.lock().ns_size }
pub fn nvme_lba_size() -> u32 { 1u32 << CTRL.lock().lba_shift }

pub fn nvme_read(lba: u64, count: u16, buf_pa: u64) -> bool {
    let mut ctrl = CTRL.lock();
    if !ctrl.ready { return false; }
    let sqe = Sqe { cdw0: NVM_READ as u32, nsid: 1, prp1: buf_pa, prp2: 0,
        cdw10: (lba & 0xFFFF_FFFF) as u32, cdw11: (lba >> 32) as u32,
        cdw12: (count as u32).saturating_sub(1), ..Default::default() };
    let cid = unsafe { ctrl.io.submit(sqe) };
    unsafe { ctrl.io.poll(cid, 4_000_000) } == 0
}

pub fn nvme_write(lba: u64, count: u16, buf_pa: u64) -> bool {
    let mut ctrl = CTRL.lock();
    if !ctrl.ready { return false; }
    let sqe = Sqe { cdw0: NVM_WRITE as u32, nsid: 1, prp1: buf_pa, prp2: 0,
        cdw10: (lba & 0xFFFF_FFFF) as u32, cdw11: (lba >> 32) as u32,
        cdw12: (count as u32).saturating_sub(1), ..Default::default() };
    let cid = unsafe { ctrl.io.submit(sqe) };
    unsafe { ctrl.io.poll(cid, 4_000_000) } == 0
...
pub fn nvme_irq() {
    ...
    unsafe { ctrl.write32(NVME_REG_INTMC, 1); ctrl.io.drain(); }
}
