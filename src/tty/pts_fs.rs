...
/// Heap-allocated data for the master side of a PTY open.
///
/// Boxed inside `PtsFd::Master` so both variants have the same stack size,
/// satisfying `clippy::large_enum_variant`.
pub struct PtsMasterData {
    pub slave_idx: u32,
    pub pair: alloc::sync::Arc<crate::tty::pty::PtyPair>,
}
...
pub enum PtsFd {
    /// Open the master side; data is boxed to keep variant sizes equal.
    Master(alloc::boxed::Box<PtsMasterData>),
...
pub fn vfs_open(path: &str) -> Result<PtsFd, isize> {
    if path == "/dev/ptmx" {
        ...
        return Ok(PtsFd::Master(alloc::boxed::Box::new(PtsMasterData {
            slave_idx: idx,
            pair,
        })));
    }
