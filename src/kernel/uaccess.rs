...
fn user_range_valid(va: usize, len: usize) -> bool {
    ...
    match va.checked_add(len) { Some(end) => end <= USER_SPACE_END, None => false }
}

fn pages_mapped(va: usize, len: usize) -> bool {
    let _guard = if crate::proc::scheduler::has_current_user_proc() { Some(crate::proc::scheduler::with_current_mm_read()) } else { None };
    let cr3 = crate::proc::scheduler::with_proc(crate::proc::scheduler::current_pid(), |p| p.user_satp).unwrap_or(0);
    if cr3 == 0 { return false; }
    let first_page = va & !(PAGE_SIZE - 1);
    let last_page  = (va + len - 1) & !(PAGE_SIZE - 1);
    let mut page   = first_page;
    while page <= last_page {
        if <Arch as Paging>::virt_to_phys(cr3, page).is_none() { return false; }
        page = match page.checked_add(PAGE_SIZE) { Some(n) => n, None => break };
    }
    true
}
...
pub fn read_path(ptr: *const u8) -> Option<alloc::string::String> {
    ...
    loop {
        ...
        if len >= next_page_check {
            ...
            if <Arch as Paging>::virt_to_phys(cr3, ps).is_none() { return None; }
