...
extern crate alloc;
use crate::time::{read_monotonic_ns, Timespec, NSEC_PER_SEC};
...
impl Wheel {
    ...
    fn expire(&mut self, now_ns: u64) {
        ...
        let fired: alloc::vec::Vec<u64> = self
            .entries
            .range(..=deadline_max)
            .map(|(&k, _)| k)
            .collect();
...
pub fn add_oneshot(deadline_ns: u64, cb: TimerCallback) -> u64 {
    WHEEL
        .lock()
        .as_mut()
        .map_or(0, |w| w.insert(deadline_ns, 0, cb))
}
...
pub fn add_periodic(first_ns: u64, period_ns: u64, cb: TimerCallback) -> u64 {
    WHEEL
        .lock()
        .as_mut()
        .map_or(0, |w| w.insert(first_ns, period_ns, cb))
}

/// Cancel a timer by its ID.
pub fn cancel_timer(id: u64) {
    if let Some(w) = WHEEL.lock().as_mut() {
        w.cancel(id);
    }
}
...
pub fn clock_nanosleep(_clk_id: i32, _flags: i32, req: Timespec) -> Result<(), isize> {
    if !req.is_valid() {
        return Err(-22);
    }
