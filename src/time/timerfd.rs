...
impl TimerFd {
    ...
    pub fn settime(&self, flags: i32, new: ItimerSpec) -> Result<ItimerSpec, isize> {
        if !new.it_value.is_valid() || !new.it_interval.is_valid() { return Err(-22); }
        // Disarm existing timer.
        let old_id = self.timer_id.swap(0, Ordering::SeqCst);
        if old_id != 0 { cancel_timer(old_id); }
        ...
        *self.current.lock() = new;
        ...
        let deadline_ns = if flags & TFD_TIMER_ABSTIME != 0 {
            ...
            let now_ns = clock::clock_gettime(self.clk_id)
                .map(|t| t.to_ns()).unwrap_or(0);
            let req_ns = new.it_value.to_ns();
            if req_ns <= now_ns { return Ok(old_spec); }
    ...
    pub fn gettime(&self) -> ItimerSpec {
        ...
        if timer_id == 0 {
            return ItimerSpec { it_interval: spec.it_interval, it_value: Timespec::ZERO };
        }
        spec
    }
    ...
    pub fn read(&self) -> Result<u64, isize> {
        loop {
            ...
            if self.nonblock { return Err(-11); } // EAGAIN
            core::hint::spin_loop();
        }
    }
}
...
pub fn init() { *REGISTRY.lock() = Some(BTreeMap::new()); }
...
pub fn timerfd_create(clk_id: i32, flags: i32) -> Result<TimerFdId, isize> {
    match clk_id {
        clock::CLOCK_REALTIME  | clock::CLOCK_REALTIME_ALARM
        | clock::CLOCK_MONOTONIC | clock::CLOCK_BOOTTIME
...
pub fn timerfd_settime(
    id: TimerFdId, flags: i32, new: ItimerSpec,
) -> Result<ItimerSpec, isize> {
    let reg = REGISTRY.lock();
