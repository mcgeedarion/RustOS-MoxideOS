...
//!   - non-pipe end uses pread/pwrite so the file offset advances correctly
//!     (or stays fixed if the caller supplied a non-NULL off_in/off_out)
...
pub fn sys_splice(
    ...
    if !in_is_pipe && !out_is_pipe { return -22; } // EINVAL
    ...
    if !in_is_pipe && out_is_pipe {
        ...
        if n <= 0 { return if nonblock && n == -11 { -11 } else { n }; }
        let written = crate::fs::pipe::pipe_write_kernel(fd_out, &buf[..n as usize]);
        if written > 0 {
            let new_off = offset + written as i64;
            if off_in_va != 0 { write_loff(off_in_va, new_off); }
            else              { advance_offset(fd_in, written as i64); }
        }
        return written;
    }
    ...
    if written > 0 {
        let new_off = offset + written as i64;
        if off_out_va != 0 { write_loff(off_out_va, new_off); }
        else               { advance_offset(fd_out, written as i64); }
    }
    written
}
