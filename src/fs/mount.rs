...
pub fn kernel_mount(
    ...
    MOUNT_TABLE
        .lock()
        .mount(source, target, fstype, flags, overlay)
}
...
pub fn init_mounts() {
    ...
    log::info!(
        "mount: root filesystem detected as '{}'",
...
pub fn sys_mount(source: &str, target: &str, fstype_s: &str, flags: u64, data: &str) -> isize {
    ...
    let overlay = if fstype == FsType::Overlayfs {
        ...
        for kv in data.split(',') {
            ...
            if let Some(v) = kv.strip_prefix("workdir=") {
                work = v.to_string();
            }
        }
        if lower.is_empty() {
    ...
    match MOUNT_TABLE
        .lock()
        .mount(source, target, fstype, flags, overlay.clone())
    {
        Err(e) => return e,
        Ok(()) => {},
    }
