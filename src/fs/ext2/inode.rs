...
impl Superblock {
    fn from_bytes(b: &[u8]) -> Option<Self> {
        ...
        if r16(56) != EXT2_SUPER_MAGIC {
            return None;
        }
        let mut uuid = [0u8; 16];
        uuid.copy_from_slice(&b[104..120]);
        let mut vn = [0u8; 16];
        vn.copy_from_slice(&b[120..136]);
        let mut lm = [0u8; 64];
        lm.copy_from_slice(&b[136..200]);
        Some(Superblock {
            inodes_count: r32(0),
            blocks_count: r32(4),
            r_blocks_count: r32(8),
            free_blocks_count: r32(12),
            free_inodes_count: r32(16),
            first_data_block: r32(20),
            log_block_size: r32(24),
            log_frag_size: r32(28),
            blocks_per_group: r32(32),
            frags_per_group: r32(36),
            inodes_per_group: r32(40),
            mtime: r32(44),
            wtime: r32(48),
            mnt_count: r16(52),
            max_mnt_count: r16(54),
            magic: r16(56),
            state: r16(58),
            errors: r16(60),
            minor_rev_level: r16(62),
            lastcheck: r32(64),
            checkinterval: r32(68),
            creator_os: r32(72),
            rev_level: r32(76),
            def_resuid: r16(80),
            def_resgid: r16(82),
            first_ino: r32(84),
            inode_size: r16(88),
            block_group_nr: r16(90),
            feature_compat: r32(92),
            feature_incompat: r32(96),
...
impl Inode {
    ...
    fn to_bytes(&self) -> Vec<u8> {
        ...
        for i in 0..15 {
            b[40 + i * 4..44 + i * 4].copy_from_slice(&self.block[i].to_le_bytes());
        }
...
impl Ext2Fs {
    ...
    fn free_block(&mut self, blkno: u32) {
        ...
        let mut gd = match self.read_group_desc(group) {
            Some(g) => g,
            None => return,
        };
        ...
        for g in 0..self.group_descs.len() {
            ...
            for i in 0..self.sb.blocks_per_group as usize {
                ...
                if bmap[byte] & (1 << bit) == 0 {
                    ...
                    self.write_group_desc(g as u32, &gd2);
                    return Some(
                        self.sb.first_data_block + g as u32 * self.sb.blocks_per_group + i as u32,
                    );
                }
            }
        }
    ...
    pub(crate) fn write_group_desc(&mut self, group: u32, gd: &BgDesc) {
        if let Some(s) = self.group_descs.get_mut(group as usize) {
            *s = gd.clone();
        }
...
impl Ext2Fs {
    ...
    fn read_raw_dir_entries(&self, dir_ino: u32) -> Vec<RawDirEntry> {
        ...
        let mut off = 0usize;
        while off + 8 <= data.len() {
            let de = match RawDirEntry::from_bytes(&data[off..]) {
                Some(d) => d,
                None => break,
            };
            if de.rec_len == 0 {
    ...
    pub(crate) fn do_readlink(&self, path: &str) -> Result<alloc::string::String, isize> {
        ...
        if inode.file_size() <= 60 {
            ...
            return Ok(
                alloc::string::String::from_utf8_lossy(&raw[..inode.file_size() as usize])
                    .into_owned(),
            );
        }
    ...
    pub(crate) fn do_unlink(&mut self, path: &str) -> Result<(), isize> {
        ...
        if i2.links_count == 0 {
            i2.dtime = crate::arch::time::current_unix_time_secs() as u32;
        }
    ...
    pub(crate) fn do_rename(&mut self, old: &str, new: &str) -> Result<(), isize> {
        ...
        let ft = if inode.mode & EXT2_S_IFMT == EXT2_S_IFDIR {
            2u8
        } else {
            1u8
        };
    ...
    pub(crate) fn do_symlink(&mut self, target: &str, path: &str) -> Result<(), isize> {
        ...
        if tb.len() <= 60 {
            let mut inode = self.read_inode(ino).ok_or(-2isize)?;
            let raw: &mut [u8] =
                unsafe { core::slice::from_raw_parts_mut(inode.block.as_mut_ptr() as *mut u8, 60) };
    ...
    pub(crate) fn do_chown(&mut self, path: &str, uid: u32, gid: u32) -> Result<(), isize> {
        ...
        let mut i = self.read_inode(ino).ok_or(-2isize)?;
        i.uid = uid as u16;
        i.uid_high = (uid >> 16) as u16;
        i.gid = gid as u16;
        i.gid_high = (gid >> 16) as u16;
        self.write_inode(ino, &i)
    }
