...
use alloc::vec::Vec;
use super::superblock::{BtrfsFs, BTRFS_MOUNTS, BtrfsSuperblock};
...
pub fn mount(subpath: &str) -> Option<()> {
    ...
    if fs.fs_tree_root == 0 { return None; }
...
fn parse_sys_chunk_array(
    ...
    use super::superblock::{BtrfsKey, BtrfsChunkItem};
    ...
    while off + 17 <= arr.len() {
        ...
        off += 17;
        if key.ty != 228 { break; } // BTRFS_CHUNK_ITEM_KEY
        let Some(chunk) = BtrfsChunkItem::from_bytes(&arr[off..]) else { break; };
...
fn resolve_fs_tree_root(fs: &BtrfsFs) -> Option<u64> {
    use super::superblock::{BtrfsKey, BTRFS_FS_TREE_OBJECTID, BTRFS_ROOT_ITEM_KEY, BtrfsRootItem};
...
fn mount_for(path: &str) -> &'static str { "/" } // single-mount stub
...
pub fn btrfs_set_times(path: &str, atime_sec: u64, mtime_sec: u64) -> Result<(), isize> {
    with_fs_mut(mount_for(path), |fs| fs.set_times(path, atime_sec, mtime_sec))?
}
