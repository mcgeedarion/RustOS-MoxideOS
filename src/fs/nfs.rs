...
impl<'a> XdrBuf<'a> {
    ...
    fn opaque_fixed(&mut self, n: usize) -> Option<Vec<u8>> {
        let padded = (n + 3) & !3;
        if self.pos + padded > self.data.len() { return None; }
    ...
    fn fattr3(&mut self) -> Option<NfsStat> {
        ...
        let fsid    = self.u64()?;
        let fileid  = self.u64()?;
        let asec    = self.u32()? as u64; let _ans = self.u32()?;
        let msec    = self.u32()? as u64; let _mns = self.u32()?;
        let csec    = self.u32()? as u64; let _cns = self.u32()?;
        Some(NfsStat { ftype, mode, nlink, uid, gid, size, used, rdev, fsid,
                       fileid, atime: asec, mtime: msec, ctime: csec })
    }
    fn post_op_attr(&mut self) -> Option<NfsStat> {
        let present = self.u32()?;
        if present == 1 { self.fattr3() } else { None }
    }
}
...
fn build_rpc_call(xid: u32, program: u32, version: u32, procedure: u32,
    ...
    xdr_u32(&mut buf, 0); xdr_u32(&mut buf, 0);
    // AUTH_NULL verifier
    xdr_u32(&mut buf, 0); xdr_u32(&mut buf, 0);
...
fn rpc_call(
    ...
    program: u32, version: u32, procedure: u32, port: u16,
    body: &[u8],
) -> Option<Vec<u8>> {
    ...
    // sendto
    let sent = crate::net::udp::udp_send_raw(
        sock, &pkt, client.server_ip, port);
    if sent < 0 { crate::fs::io_syscalls::sys_close(sock); return None; }
...
fn portmap_getport(server_ip: u32, program: u32, version: u32, proto: u32) -> Option<u16> {
    ...
    let reply = rpc_call(&mut dummy, PROGRAM_PORTMAP, VERSION_PORTMAP, PORTMAP_GETPORT, 111, &body)?;
    // reply: xid(4) + REPLY(4) + MSG_ACCEPTED(4) + verifier(8) + SUCCESS(4) + port(4) = offset 28
    if reply.len() < 32 { return None; }
...
pub fn mount(server_ip: u32, export_path: &str) -> bool {
    ...
    let mount_port = portmap_getport(server_ip, PROGRAM_MOUNT, VERSION_MOUNT, 17)
        .unwrap_or(635);
    let nfs_port   = portmap_getport(server_ip, PROGRAM_NFS,   VERSION_NFS,   17)
        .unwrap_or(2049);

    // 2. Call MOUNTD MNT to get root file handle.
    let mut client = NfsClient {
        server_ip, nfs_port, root_fh: Fh::default(), xid: 0x2000,
    };
    let mut body = Vec::new();
    xdr_string(&mut body, export_path);
    let reply = match rpc_call(&mut client, PROGRAM_MOUNT, VERSION_MOUNT,
                               MOUNT_MNT, mount_port, &body) {
        Some(r) => r,
        None => return false,
    };
    // Parse reply: xid(4) REPLY(4) MSG_ACCEPTED(4) verifier(8) SUCCESS(4) = 24
    // then mountstat3(4) + fh(var) + auth_flavors(var)
    if reply.len() < 32 { return false; }
    let mut xdr = XdrBuf::new(&reply[24..]);
    let status = match xdr.u32() { Some(s) => s, None => return false };
    if status != NFS3_OK { return false; }
    let fh_data = match xdr.opaque_var() { Some(d) => d, None => return false };

    client.root_fh = Fh { data: fh_data };
    *CLIENT.lock() = Some(client);
    log::info!("nfs: mounted {}:{} port={}", server_ip, export_path, nfs_port);
    true
}
...
pub fn lookup(dir_fh: &Fh, name: &str) -> Option<Fh> {
    ...
    if reply.len() < 28 { return None; }
    let mut xdr = XdrBuf::new(&reply[24..]);
    let status = xdr.u32()?;
    if status != NFS3_OK { return None; }
...
pub fn getattr(fh: &Fh) -> Option<NfsStat> {
    ...
    if reply.len() < 28 { return None; }
    let mut xdr = XdrBuf::new(&reply[24..]);
    if xdr.u32()? != NFS3_OK { return None; }
    xdr.fattr3()
}
...
pub fn create(dir_fh: &Fh, name: &str, mode: u32) -> Option<Fh> {
    ...
    xdr_u32(&mut body, 1); xdr_u32(&mut body, mode);
    xdr_u32(&mut body, 0); xdr_u32(&mut body, 0); xdr_u32(&mut body, 0);
    xdr_u32(&mut body, 0); xdr_u32(&mut body, 0);
...
pub fn remove(dir_fh: &Fh, name: &str) -> bool {
    let mut guard = CLIENT.lock();
    let c = match guard.as_mut() { Some(c) => c, None => return false };
    let mut body = Vec::new();
    dir_fh.xdr_encode(&mut body);
    xdr_string(&mut body, name);
...
pub fn symlink(dir_fh: &Fh, name: &str, target: &str) -> bool {
    ...
    xdr_string(&mut body, name);
    // sattr3 all absent
    for _ in 0..5 { xdr_u32(&mut body, 0); }
...
pub fn readlink(fh: &Fh) -> Option<String> {
    ...
    let mut body = Vec::new();
    fh.xdr_encode(&mut body);
    let reply = rpc_call(c, PROGRAM_NFS, VERSION_NFS, NFS3_READLINK, c.nfs_port, &body)?;
    if reply.len() < 28 { return None; }
    let mut xdr = XdrBuf::new(&reply[24..]);
    if xdr.u32()? != NFS3_OK { return None; }
...
pub fn readdirplus(dir_fh: &Fh) -> Vec<NfsDirEntry> {
    ...
    loop {
        let reply = {
            let mut guard = CLIENT.lock();
            let c = match guard.as_mut() { Some(c) => c, None => break };
            let mut body = Vec::new();
            dir_fh.xdr_encode(&mut body);
            ...
            match rpc_call(c, PROGRAM_NFS, VERSION_NFS, NFS3_READDIRPLUS, c.nfs_port, &body) {
                Some(r) => r,
                None => break,
            }
        };
        if reply.len() < 28 { break; }
        let mut xdr = XdrBuf::new(&reply[24..]);
        if xdr.u32().unwrap_or(1) != NFS3_OK { break; }
        let _dir_attrs = xdr.post_op_attr();
        let cv: Vec<u8> = match xdr.opaque_fixed(8) { Some(v) => v, None => break };
        cookieverf.copy_from_slice(&cv);

        let mut got_any = false;
        loop {
            let value_follows = match xdr.u32() { Some(v) => v, None => break };
            if value_follows == 0 { break; }
            let fileid = match xdr.u64() { Some(v) => v, None => break };
            let name   = match xdr.string() { Some(s) => s, None => break };
            let ck     = match xdr.u64() { Some(v) => v, None => break };
            cookie = ck;
            ...
            let fh = {
                ...
                if present == 1 { xdr.opaque_var().map(|d| Fh { data: d }) } else { None }
            };
            entries.push(NfsDirEntry { fileid, name, cookie: ck, fh, attrs });
            got_any = true;
        }
        // EOF
        let eof = xdr.u32().unwrap_or(1);
        if eof == 1 || !got_any { break; }
    }
    entries
}
...
pub fn fsstat(root_fh: &Fh) -> Option<NfsFsStat> {
    ...
    Some(NfsFsStat {
        total_bytes: tbytes, free_bytes: fbytes, avail_bytes: abytes,
        total_files: tfiles, free_files: ffiles,
    })
}
