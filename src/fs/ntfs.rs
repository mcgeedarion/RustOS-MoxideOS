...
impl NtfsVol {
    ...
    fn fixup_record(&self, rec: &mut [u8; MFT_RECORD_SIZE]) {
        ...
        if usn_off + usn_cnt * 2 > MFT_RECORD_SIZE {
            return;
        }
        // usn_cnt includes the USN itself in position [0], then (usn_cnt-1)
        // replacements
        for i in 1..usn_cnt {
    ...
    /// Read all data bytes of a file record (handles resident and non-resident
    /// $DATA).
            ...
            if non_res == 0 {
                ...
                for (len_clusters, delta) in decoded {
                    ...
                    for c in 0..len_clusters {
                        ...
                        if let Some(slice) =
                            self.data.get(byte_off..byte_off + self.bytes_per_cluster)
                        {
    ...
    fn resolve_path(&self, path: &str) -> Option<u64> {
        ...
        for comp in &components {
            ...
            let found = entries
                .into_iter()
                .find(|(n, ..)| n.eq_ignore_ascii_case(comp))?;
            cur_rec_no = found.1;
        }
        Some(cur_rec_no)
    }
...
pub fn mount(image: Vec<u8>) -> bool {
    ...
    *VOL.lock() = Some(NtfsVol {
    ...
    log::info!(
        "ntfs: mounted volume (ro), MFT at byte offset {}",
        mft_offset
    );
    true
}
