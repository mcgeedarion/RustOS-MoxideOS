...
use crate::exec::elf::{
    Elf64Hdr, Elf64Phdr,
    parse_elf_header, parse_phdrs_with_hdr,
...
pub fn parse_notes( &[u8]) -> Vec<ElfNote> {
    ...
    while pos + 12 <= data.len() {
        ...
        let descsz = u32::from_le_bytes(data[pos+4..pos+8].try_into().unwrap_or([0;4])) as usize;
        ...
        if pos + name_padded + desc_padded > data.len() { break; }
        ...
        notes.push(ElfNote { name, note_type: ntype, desc });
    }
    notes
}
...
/// Encode a list of `NtFileEntry` records into a raw NT_FILE note `desc` payload.
/// Format matches Linux: count, page_size, then (start, end, ofs) * n, then names.
