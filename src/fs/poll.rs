...
fn read_fdset(va: usize, words: usize) -> Result<Vec<u64>, isize> {
    ...
    if !validate_user_ptr(va, words * 8) {
        return Err(-14);
    }
...
fn pselect_swap_sigmask(ss_va: usize) -> Option<u64> {
    ...
    if !validate_user_ptr(ss_va, 16) {
        return None;
    }
    let mut arg_buf = [0u8; 16];
    if copy_from_user(&mut arg_buf, ss_va).is_err() {
        return None;
    }
    let ss_ptr = u64::from_le_bytes(arg_buf[0..8].try_into().unwrap());
    let ss_len = u64::from_le_bytes(arg_buf[8..16].try_into().unwrap());
    if ss_ptr == 0 || ss_len != 8 {
        return None;
    }
    if !validate_user_ptr(ss_ptr as usize, 8) {
        return None;
    }
    let mut mask_buf = [0u8; 8];
    if copy_from_user(&mut mask_buf, ss_ptr as usize).is_err() {
        return None;
    }
...
pub fn sys_select(
    ...
    for va in [rfds_va, wfds_va, efds_va] {
        if va != 0 && !validate_user_ptr(va, bmap_size) {
            return -14;
        }
    }
        ...
        match read_timeval(tv_va) {
            ...
            Ok((sec, usec)) => {
                ...
                let dl = if zero {
                    ...
                    now_ns() + wait_ns.min(5_000_000_000)
                };
                (dl, zero)
            }
        }
    ...
    let rfds_k = match read_fdset(rfds_va, words) {
        Ok(v) => v,
        Err(e) => return e,
    };
    let wfds_k = match read_fdset(wfds_va, words) {
        Ok(v) => v,
        Err(e) => return e,
    };
    // FIX: read efds from efds_va, not derived from rfds/wfds.
    let efds_k = match read_fdset(efds_va, words) {
        Ok(v) => v,
        Err(e) => return e,
    };
...
pub fn sys_poll(fds_va: usize, nfds: usize, timeout_ms: i32) -> isize {
    ...
    if !validate_user_ptr(fds_va, nfds * struct_size) {
        return -14;
    }
    ...
    for i in 0..nfds {
        ...
        let mut raw = [0u8; 8];
        if copy_from_user(&mut raw, pfd_va).is_err() {
            return -14;
        }
    ...
    loop {
        ...
        for (i, pfc) in pfds.iter().enumerate() {
            ...
            if copy_to_user(revents_va, &rev.to_le_bytes()).is_err() {
                return -14;
            }
            if rev != 0 {
                total += 1;
...
static EPOLL_TABLE: Mutex<[Option<Epoll>; MAX_EPOLLS]> = Mutex::new([const { None }; MAX_EPOLLS]);
...
fn epoll_idx(epfd: usize) -> Option<usize> {
    if epfd < EPOLL_FD_BASE || epfd >= EPOLL_FD_BASE + MAX_EPOLLS {
        return None;
    }
...
pub fn sys_epoll_ctl(epfd: usize, op: i32, target_fd: i32, event_va: usize) -> isize {
    ...
    let (events, data): (u32, u64) = if op != EPOLL_CTL_DEL {
        if !validate_user_ptr(event_va, 12) {
            return -14;
        }
        let mut buf = [0u8; 12];
        if copy_from_user(&mut buf, event_va).is_err() {
            return -14;
        }
    ...
    match op {
        EPOLL_CTL_ADD => {
            if ep.entries.iter().any(|e| e.fd == target_fd) {
                return -17;
            }
            ep.entries.push(EpollEntry {
                fd: target_fd,
                events,
                data,
            });
        ...
        EPOLL_CTL_MOD => match ep.entries.iter_mut().find(|e| e.fd == target_fd) {
            Some(e) => {
                e.events = events;
                e.data = data;
                0
...
pub fn sys_epoll_wait(epfd: usize, events_va: usize, maxevents: i32, timeout_ms: i32) -> isize {
    ...
    if !validate_user_ptr(events_va, maxevents as usize * 12) {
        return -14;
    }
    ...
    loop {
...

        {
            ...
            for entry in &ep.entries {
                ...
                if copy_to_user(out_va, &rec).is_err() {
                    return -14;
                }
                found += 1;
