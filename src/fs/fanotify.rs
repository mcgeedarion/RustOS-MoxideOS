...
pub fn sys_fanotify_init(flags: u32, _event_f_flags: u32) -> isize {
    ...
    TABLE.lock().insert(table_fdno, FanotifyInstance {
        class, flags, nonblock,
        queue: Vec::new(),
        marks: Vec::new(),
    });
...
pub fn sys_fanotify_mark(
    scheme_bfd: usize, flags: u32, mask: u64, _dirfd: i32, path_va: usize,
) -> isize {
    if flags & FAN_MARK_FLUSH != 0 {
        ...
        if let Some(inst) = tbl.get_mut(&fdno) { inst.marks.clear(); }
        return 0;
    }
    ...
    if flags & FAN_MARK_ADD != 0 {
        ...
        for m in inst.marks.iter_mut() {
            if m.path == path {
                if flags & FAN_MARK_IGNORED_MASK != 0 { m.ignored |= mask; }
                else { m.mask |= mask; }
                return 0;
            }
        }
