...
/// Parse file/directory name from a directory record, stripping the `;1` version suffix.
fn parse_name(rec: &[u8]) -> String {
    ...
    if raw == [0x00] { return ".".to_string(); }
    if raw == [0x01] { return "..".to_string(); }
...
fn list_dir(img: &[u8], lba: u32, size: u32) -> Vec<(String, u32, u32, bool)> {
    ...
    while pos < end {
        ...
        let rec   = match img.get(pos..pos + rec_len) { Some(r) => r, None => break };
...
fn resolve_path(img: &CdImage, path: &str) -> Option<(u32, u32, bool)> {
    ...
    for (i, comp) in components.iter().enumerate() {
        ...
        let found = entries.into_iter().find(|(name, ..)| name.eq_ignore_ascii_case(comp))?;
        if i == components.len() - 1 {
            return Some((found.1, found.2, found.3));
        }
        if !found.3 { return None; } // not a directory
        cur_lba  = found.1;
        cur_size = found.2;
    }
    None
...
pub fn mount(image_data: Vec<u8>) -> bool {
    ...
    if pvd[0] != PVD_TYPE_PRIMARY { return false; }
    if &pvd[PVD_MAGIC_OFFSET..PVD_MAGIC_OFFSET + 5] != PVD_MAGIC { return false; }
    ...
    *IMAGE.lock() = Some(CdImage { data: image_data, root_lba, root_len });
    log::info!("cdfs: mounted ISO 9660 image, root LBA={} size={}", root_lba, root_len);
    true
}
...
pub fn read_file(path: &str) -> Result<Vec<u8>, isize> {
    ...
    if is_dir { return Err(-21); } // EISDIR
    let start = lba as usize * SECTOR_SIZE;
    let end   = start + size as usize;
    img.data.get(start..end)
        .map(|s| s.to_vec())
        .ok_or(-5)
}
...
pub fn read_dir(path: &str) -> Result<Vec<(String, bool)>, isize> {
    ...
    if !is_dir { return Err(-20); } // ENOTDIR
