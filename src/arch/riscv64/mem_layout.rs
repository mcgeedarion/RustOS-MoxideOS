...
pub mod sv39 {
    ...
    #[inline]
    pub const fn vpn0(va: usize) -> usize {
        (va >> 12) & 0x1FF
    }
    /// Extract VPN[1] (PD index, bits [29:21]).
    #[inline]
    pub const fn vpn1(va: usize) -> usize {
        (va >> 21) & 0x1FF
    }
    /// Extract VPN[2] (root index, bits [38:30]).
    #[inline]
    pub const fn vpn2(va: usize) -> usize {
        (va >> 30) & 0x1FF
    }
...
pub mod sie {
    ...
    /// Also the bit to clear in `sip` to acknowledge an S-mode software
    /// interrupt.
...
pub mod trap {
    /// Number of 8-byte slots saved in the trap frame (32 GPRs + sepc + sstatus
    /// + pad).
