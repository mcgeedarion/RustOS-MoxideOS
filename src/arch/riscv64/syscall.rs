...
/// SBI function IDs for the Hart State Management extension (EID =
/// `sbi_eid::HSM`).
pub mod sbi_fid_hsm {
...
pub fn sbi_remote_sfence_vma(
    ...
    unsafe {
        sbi_call(
            sbi_eid::RFNC,
            sbi_fid_rfnc::REMOTE_SFENCE_VMA,
            hart_mask,
            hart_mask_base,
            start,
            size,
...
pub fn sbi_console_write_byte(byte: u8) -> SbiRet {
    unsafe {
        sbi_call1(
            sbi_eid::DBCN,
            sbi_fid_dbcn::CONSOLE_WRITE_BYTE,
            byte as usize,
        )
    }
}
