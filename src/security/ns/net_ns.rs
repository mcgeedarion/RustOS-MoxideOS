...
impl NetNs {
    ...
    pub fn new_init() -> Self {
        let ns = NetNs {
            ...
            routes4:      Mutex::new(alloc::vec![
                Route4 { dst: 0x7F00_0000, mask: 0xFF00_0000,
                         gateway: 0, ifindex: 1, metric: 0, flags: 1 },
            ]),
    ...
    pub fn remove_iface(&self, ifindex: u32) -> Result<(), isize> {
        ...
        let pos = list.iter().position(|i| i.ifindex == ifindex).ok_or(-19isize)?;
        list.remove(pos);
        Ok(())
    }
    ...
    pub fn ifindex_by_name(&self, name: &str) -> Option<u32> {
        self.ifaces.lock().iter().find(|i| i.name == name).map(|i| i.ifindex)
    }

    /// Set interface flags (SIOCSIFFLAGS).
    pub fn set_flags(&self, ifindex: u32, flags: u32) -> Result<(), isize> {
        let mut list = self.ifaces.lock();
        let iface = list.iter_mut().find(|i| i.ifindex == ifindex).ok_or(-19isize)?;
        iface.flags = flags;
        Ok(())
    }
    ...
    pub fn iface_list(&self) -> Vec<NetIface> { self.ifaces.lock().clone() }

    /// Snapshot of all IPv4 routes (/proc/net/route).
    pub fn route_list(&self) -> Vec<Route4> { self.routes4.lock().clone() }
