...
pub fn alloc_ns_id() -> u64 { NEXT_NS_ID.fetch_add(1, Ordering::SeqCst) }
...
impl NsSet {
    ...
    pub fn clone_with_flags(&self, flags: u64) -> Self {
        NsSet {
            pid:  if flags & CLONE_NEWPID  != 0 { Arc::new(pid_ns::PidNs::new_child()) }
                  else { self.pid.clone() },
            mnt:  if flags & CLONE_NEWNS   != 0 { Arc::new(mnt_ns::MntNs::copy_of(&self.mnt)) }
                  else { self.mnt.clone() },
            net:  if flags & CLONE_NEWNET  != 0 { Arc::new(net_ns::NetNs::new_empty()) }
                  else { self.net.clone() },
            uts:  if flags & CLONE_NEWUTS  != 0 { Arc::new(uts_ns::UtsNs::copy_of(&self.uts)) }
                  else { self.uts.clone() },
            user: if flags & CLONE_NEWUSER != 0 { Arc::new(user_ns::UserNs::new_child(&self.user)) }
                  else { self.user.clone() },
        }
    }
}
