...
impl UserNs {
    pub fn new_init() -> Self {
        ...
        UserNs {
            id:             alloc_ns_id(),
            parent:         None,
            uid_map:        Mutex::new(alloc::vec![IdMapEntry { ns_start: 0, host_start: 0, count: u32::MAX }]),
            gid_map:        Mutex::new(alloc::vec![IdMapEntry { ns_start: 0, host_start: 0, count: u32::MAX }]),
    ...
    pub fn write_uid_map(&self, entries: Vec<IdMapEntry>) -> Result<(), isize> {
        ...
        if *mapped { return Err(-1); } // EPERM: already written
        if entries.len() > MAX_MAP_ENTRIES { return Err(-22); }
