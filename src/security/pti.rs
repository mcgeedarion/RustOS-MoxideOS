...
impl PtiCr3Pair {
    ...
    pub fn kernel_cr3_val(&self, _hint: bool) -> u64 {
        let f = if PTI_PCID_ENABLED.load(Ordering::Relaxed) { CR3_NO_FLUSH } else { 0 };
...
pub fn build_shadow_pml4(
    ...
    unsafe {
        ...
        map_trampoline_into_shadow(
            shadow_ptr, trampoline_va, trampoline_phys,
...
unsafe fn map_trampoline_into_shadow(
    pml4_virt: *mut u64, va: u64, pa: u64, leaf_flags: u64,
) {
...
#[inline] fn phys_to_virt(phys: u64) -> u64 { phys + PHYS_OFFSET }
