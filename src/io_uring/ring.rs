...
//!
//! Each call to `io_uring_setup` allocates two PMM pages:
//!   - one page for the SQ ring header + SQE array
//!   - one page for the CQ ring header + CQE array
//!
//! Both pages are mapped into the calling process's address space by
//! `mmap` — the user receives the VA via the `sq_off`/`cq_off` fields
//! in `IoUringParams`.  The kernel always accesses them through the
//! kernel-virtual (identity-mapped) PA.
...
/// Maximum SQE/CQE depth for a single ring (power of two).
pub const MAX_ENTRIES:  u32 = 4096;
/// Maximum number of simultaneous rings across all processes.
...
/// Page size — rings are always a whole number of pages.
const PAGE_SIZE: usize = 4096;

// ── Wire structures (ABI-compatible with Linux) ───────────────────────────────

/// Submission Queue Entry — 64 bytes, matches Linux's `struct io_uring_sqe`.
#[repr(C)]
...
/// Completion Queue Entry — 16 bytes, matches Linux's `struct io_uring_cqe`.
#[repr(C)]
#[derive(Clone, Copy, Default)]
...
// ── SQ/CQ ring in-memory layout ───────────────────────────────────────────────
//
// We store SQ metadata at byte 0 of the SQ page, then the SQE array at a
// fixed offset (SQRING_HDR_SIZE).  The CQ page follows the same pattern.
//
// Layout of SQ page (4 KiB):
//   offset  0  : SqRingHdr  (64 bytes)
//   offset 64  : u32 sq_array[entries]  (indirect index, each u32 → SQE slot)
//   offset 64+entries*4: IoUringSqe[entries]
//
// Layout of CQ page (4 KiB):
...
pub struct IoUringRing {
    ...
    pub cq_pa:       usize,
    /// Registered buffers (va, len) pairs.
...
impl IoUringRing {
    ...
    #[inline]
    pub fn mask(&self) -> u32 { self.entries - 1 }

    // ── Dequeue one SQE from the ring ─────────────────────────────────────────

    /// Drain all pending SQEs from the ring and return them.
    /// Updates `sq_head` after reading.
    ...
    /// Append a completion event to the CQ ring.
    /// Returns `false` if the CQ ring is full (overflow).
    pub fn post_cqe(&self, user_data: u64, res: i32, flags: u32) -> bool {
        ...
        let cqe  = &mut self.cqe_array()[slot];
    ...
    // ── Build IoUringParams for userspace ─────────────────────────────────────
    ...
    pub fn build_params(&self) -> IoUringParams {
        ...
        // SQ offsets — where within the SQ page each field lives.
...
// ── Offset helpers (byte offset of each atomic field in SqRingHdr/CqRingHdr) ──
...
impl RingTable {
    ...
    fn insert(&mut self, ring: IoUringRing) -> Option<usize> {
        let slot = self.rings.iter().position(|r| r.is_none())?;
        self.rings[slot] = Some(ring);
        Some(slot)
    }

    fn remove(&mut self, idx: usize) -> Option<IoUringRing> {
        self.rings.get_mut(idx)?.take()
    }

    pub fn get(&self, idx: usize) -> Option<&IoUringRing> {
        self.rings.get(idx)?.as_ref()
    }

    pub fn get_mut(&mut self, idx: usize) -> Option<&mut IoUringRing> {
...
/// Initialise the global ring table.  Called once from `mm::init()`.
pub fn init() {
...
/// Allocate a new ring with `entries` slots (rounded up to the next power of two).
///
/// Returns the table index (used as the fd slot) or an error isize.
pub fn alloc_ring(pid: u32, entries: u32) -> Result<usize, isize> {
    let entries = entries.next_power_of_two().clamp(1, MAX_ENTRIES);

    // Each ring needs at minimum 1 page for SQ and 1 for CQ.
    // We check that the arrays fit within a single page each.
    let sq_needed = RING_HDR_SIZE
        + entries as usize * 4           // sq_array
        + entries as usize * SQE_SIZE;   // SQE array
    let cq_needed = RING_HDR_SIZE
        + entries as usize * CQE_SIZE;   // CQE array
    if sq_needed > PAGE_SIZE || cq_needed > PAGE_SIZE {
        return Err(-22); // EINVAL — too many entries for a 4 KiB page
    }

    let sq_pa = alloc_page().ok_or(-12isize)? as usize; // ENOMEM
...
pub fn free_ring(idx: usize) {
    if let Some(mut tbl) = RING_TABLE.lock().as_mut() {
        tbl.remove(idx);
    }
}
