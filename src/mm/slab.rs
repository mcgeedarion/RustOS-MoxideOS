...
unsafe fn canary_check(slot: *mut u8, obj_size: usize) {
    ...
    assert_eq!(lo, CANARY_MAGIC, "slab: head canary corrupted at {:p} (got {:#010x})", slot, lo);
    assert_eq!(hi, CANARY_MAGIC, "slab: tail canary corrupted at {:p} (got {:#010x})", slot, hi);
}
