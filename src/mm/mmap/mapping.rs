...
use super::{Vma, VmaKind, PAGE, MAP_ANON, MAP_FIXED, MAP_GROWSDOWN, PROT_EXEC, PROT_WRITE};
...
pub fn sys_mmap(
    ...
    if is_anon {
        ...
        insert_vma(pid, Vma {
            start: va, end: va + len, prot, flags, kind,
            file_offset: 0, locked: false,
        });
    } else {
        insert_vma(pid, Vma {
            start: va, end: va + len, prot, flags,
            kind: VmaKind::FileBacked(fd, offset as u64),
            file_offset: offset as u64, locked: false,
        });
    }
    va as isize
}
...
#[inline]
fn va_end_of(va: usize, len: usize) -> usize { va.saturating_add(len) }
...
fn remove_vma_inner(p: &mut crate::proc::process::Pcb, addr: usize, end: usize) {
    ...
    while i < p.vmas.len() {
        ...
        if has_leading && has_trailing {
            ...
            tail.start = end;
            if let VmaKind::FileBacked(_, ref mut off) = tail.kind { *off += (end - vstart) as u64; }
            tail.file_offset = p.vmas[i].file_offset + (end - vstart) as u64;
            p.vmas[i].end = addr;
            p.vmas.insert(i + 1, tail);
            i += 2;
        } else if has_leading {
            p.vmas[i].end = addr; i += 1;
        } else {
            let delta = end - vstart;
            p.vmas[i].start = end;
            if let VmaKind::FileBacked(_, ref mut off) = p.vmas[i].kind { *off += delta as u64; }
...
fn mmap_phys(addr: usize, len: usize, prot: u32, flags: u32, pid: usize, offset: u64) -> isize {
    ...
    insert_vma(pid, Vma {
        start: va, end: va + len, prot: prot & !PROT_EXEC, flags,
        kind: VmaKind::PhysMap(offset), file_offset: offset, locked: false,
    });
    va as isize
}