...
pub fn clone_vmas(src_pid: usize, dst_pid: usize) {
    let src_vmas: Vec<Vma> = scheduler::with_proc(src_pid, |p| p.vmas.clone()).unwrap_or_default();
...
pub fn sys_munmap(addr: usize, length: usize) -> isize {
    if addr & (PAGE - 1) != 0 {
        return -22;
    } // EINVAL: not page-aligned
    if length == 0 {
        return -22;
    } // EINVAL: Linux do_munmap()
    let len = page_align_up(length);
    ...
    let phys_ranges: Vec<(usize, usize)> = scheduler::with_proc(pid, |p| {
        p.vmas
            .iter()
            .filter(|v| matches!(v.kind, VmaKind::PhysMap(_)) && v.start < end && v.end > addr)
...
pub fn sys_brk(addr: usize) -> isize {
    ...
    let (brk_base, old_brk, user_cr3) =
        scheduler::with_proc(pid, |p| (p.brk_base, p.brk, p.user_satp)).unwrap_or((0, 0, 0));
    ...
    let heap_pte_flags = PageFlags::PRESENT | PageFlags::WRITE | PageFlags::USER | PageFlags::NX;
    ...
    scheduler::with_proc(pid, |p| p.brk).unwrap_or(old_brk) as isize
}
