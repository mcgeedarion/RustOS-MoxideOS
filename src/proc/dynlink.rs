...
/// Map PT_LOAD segments into user space. Returns load bias (ET_DYN) or 0.
fn map_elf_phdrs(elf: &[u8]) -> Result<usize, isize> {
    ...
    // For ET_DYN interpreters, load at a fixed high address away from the
    // main binary. ET_EXEC interpreters use their own p_vaddr directly.
    ...
    // Get current process CR3 via the Paging trait — do not use arch internals.
    let pid = crate::proc::scheduler::current_pid();
    let cr3 = crate::proc::scheduler::with_procs(|procs| {
        procs.iter().find(|p| p.pid == pid).map_or(0, |p| p.user_satp)
    });
    if cr3 == 0 { return Err(-12); } // ENOMEM: no address space
    ...
    for i in 0..e_phnum {
        ...
        // Use PageFlags + Paging trait — never raw usize PTE constants.
        let pte_flags = {
            let mut f = PageFlags::PRESENT | PageFlags::USER;
            if writable { f |= PageFlags::WRITE; }
            // No NX on executable segments; NX on data.
            if p_flags & 1 == 0 { f |= PageFlags::NX; }
            f
        };
