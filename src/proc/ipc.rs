...
impl MsgQueue {
    fn new() -> Self {
        MsgQueue { msgs: VecDeque::new(), msgmax: 8192, msgmnb: 16384, bytes: 0 }
    }
}

struct MsgTable { inner: BTreeMap<(NsId, i32 /*msqid*/), MsgQueue>, next_id: i32 }
impl MsgTable {
    const fn new() -> Self { MsgTable { inner: BTreeMap::new(), next_id: 1 } }
...
pub fn sys_msgsnd(msqid: i32, msgp_va: usize, msgsz: usize, _msgflg: i32) -> isize {
    if msgsz > 8192 { return -22; }
    let pid = crate::proc::scheduler::current_pid();
    let ns  = crate::proc::scheduler::with_proc(pid, |p| p.ns.ipc).unwrap_or(INIT_NS);
    let mut hdr = [0u8; 8];
    if copy_from_user(&mut hdr, msgp_va).is_err() { return -14; }
    let mtype = i64::from_le_bytes(hdr);
    if mtype <= 0 { return -22; }
    let mut data = alloc::vec![0u8; msgsz];
    if msgsz > 0 && copy_from_user(&mut data, msgp_va + 8).is_err() { return -14; }
    let mut tbl = MSGQ.lock();
    if let Some(q) = tbl.inner.get_mut(&(ns, msqid)) {
        if q.bytes + msgsz > q.msgmnb { return -11; } // EAGAIN
        q.bytes += msgsz;
...
pub fn sys_msgrcv(msqid: i32, msgp_va: usize, msgsz: usize, msgtyp: i64, _msgflg: i32) -> isize {
    ...
    let ns  = crate::proc::scheduler::with_proc(pid, |p| p.ns.ipc).unwrap_or(INIT_NS);
    let mut tbl = MSGQ.lock();
    let q = match tbl.inner.get_mut(&(ns, msqid)) { Some(q) => q, None => return -22 };
    ...
    match pos {
        ...
        Some(i) => {
            ...
            if copy_to_user(msgp_va, &msg.mtype.to_le_bytes()).is_err() { return -14; }
...
struct SemSet { vals: Vec<i32> }

struct SemTable { inner: BTreeMap<(NsId, i32), SemSet>, next_id: i32 }
impl SemTable {
    const fn new() -> Self { SemTable { inner: BTreeMap::new(), next_id: 1 } }
}
...
pub fn sys_semget(key: IpcKey, nsems: i32, semflg: u32) -> isize {
    if nsems < 0 || nsems > 128 { return -22; }
    let pid = crate::proc::scheduler::current_pid();
    let ns  = crate::proc::scheduler::with_proc(pid, |p| p.ns.ipc).unwrap_or(INIT_NS);
    ...
    if semflg & IPC_CREAT != 0 {
        let id = tbl.next_id;
        tbl.next_id += 1;
        tbl.inner.insert((ns, id), SemSet { vals: alloc::vec![0i32; nsems as usize] });
        return id as isize;
    }
...
pub fn sys_semop(semid: i32, sops_va: usize, nsops: usize) -> isize {
    if nsops == 0 { return 0; }
    if nsops > 32  { return -22; }
    let pid = crate::proc::scheduler::current_pid();
    let ns  = crate::proc::scheduler::with_proc(pid, |p| p.ns.ipc).unwrap_or(INIT_NS);
    let mut ops_buf = alloc::vec![0u8; nsops * 6];
    if copy_from_user(&mut ops_buf, sops_va).is_err() { return -14; }
    let mut tbl = SEMS.lock();
    let set = match tbl.inner.get_mut(&(ns, semid)) { Some(s) => s, None => return -22 };
    for i in 0..nsops {
        ...
        if sem_num >= set.vals.len() { return -22; }
        let new_val = set.vals[sem_num] + sem_op;
        if new_val < 0 { return -11; } // EAGAIN (would block)
...
pub fn sys_semctl(semid: i32, semnum: i32, cmd: i32, arg: usize) -> isize {
    ...
    let mut tbl = SEMS.lock();
    match cmd {
        x if x == IPC_RMID => { tbl.inner.remove(&(ns, semid)); 0 }
        x if x == SETVAL => {
            let set = match tbl.inner.get_mut(&(ns, semid)) { Some(s) => s, None => return -22 };
            if semnum < 0 || semnum as usize >= set.vals.len() { return -22; }
            set.vals[semnum as usize] = arg as i32;
            0
        }
        x if x == GETVAL => {
            let set = match tbl.inner.get(&(ns, semid)) { Some(s) => s, None => return -22 };
            if semnum < 0 || semnum as usize >= set.vals.len() { return -22; }
        ...
        x if x == SETALL => {
            ...
            for i in 0..set.vals.len() {
                let mut v = [0u8; 2];
                if copy_from_user(&mut v, arg + i * 2).is_err() { return -14; }
        ...
        x if x == GETALL => {
            ...
            for (i, &v) in set.vals.iter().enumerate() {
                ...
                if copy_to_user(arg + i * 2, &bytes).is_err() { return -14; }
            }
            0
        }
...
struct ShmTable { inner: BTreeMap<(NsId, i32), ShmRegion>, next_id: i32 }
impl ShmTable {
    const fn new() -> Self { ShmTable { inner: BTreeMap::new(), next_id: 1 } }
}

static SHMS: Mutex<ShmTable> = Mutex::new(ShmTable::new());

/// Active shmat attachments: pid → Vec<(shmid, user_va, size)>
static SHM_ATTACH: Mutex<BTreeMap<usize, Vec<(i32, usize, usize)>>> =
...
pub fn sys_shmget(key: IpcKey, size: usize, shmflg: u32) -> isize {
    ...
    if shmflg & IPC_CREAT != 0 {
        let id = tbl.next_id;
        tbl.next_id += 1;
        tbl.inner.insert((ns, id), ShmRegion {
...
pub fn sys_shmat(shmid: i32, shmaddr: usize, shmflg: u32) -> isize {
    ...
    let tbl = SHMS.lock();
    let region = match tbl.inner.get(&(ns, shmid)) { Some(r) => r, None => return -22 };
    let size = region.data.len();
    let rounded = (size + PAGE - 1) & !(PAGE - 1);
    drop(tbl);

    // Allocate a user VA range.
    let va = if shmaddr != 0 && shmflg & 0x4000 != 0 /* SHM_REMAP */ {
        shmaddr
    } else {
    ...
    // Register attachment.
    SHM_ATTACH.lock().entry(pid).or_default().push((shmid, va, rounded));
    va as isize
}
    ...
    match cmd {
        x if x == IPC_RMID => {
            ...
            if let Some(r) = tbl.inner.get_mut(&(ns, shmid)) {
                ...
                let has_attach = SHM_ATTACH.lock()
                    .values()
                    .any(|v| v.iter().any(|(id, _, _)| *id == shmid));
                if !has_attach { tbl.inner.remove(&(ns, shmid)); }
            }
            0
        }
        x if x == IPC_STAT => 0,
...
pub fn sys_mq_open(name_va: usize, oflag: u32, _mode: u32, attr_va: usize) -> isize {
    ...
    if let Some(&mqd) = tbl.by_name.get(&key) {
        if oflag & O_EXCL != 0 && oflag & O_CREAT != 0 { return -17; } // EEXIST
        return mqd as isize;
    }
    if oflag & O_CREAT == 0 { return -2; } // ENOENT

    let (maxmsg, msgsize) = if attr_va != 0 {
        let mut buf = [0u8; 32];
        if copy_from_user(&mut buf, attr_va).is_err() { return -14; }
    ...
    tbl.by_mqd.insert(mqd, PosixMq { msgs: Vec::new(), maxmsg, msgsize });
    mqd as isize
}
...
pub fn sys_mq_send(mqdes: i32, buf_va: usize, msg_len: usize, msg_prio: u32) -> isize {
    let mut tbl = POSIX_MQ.lock();
    let mq = match tbl.by_mqd.get_mut(&mqdes) { Some(m) => m, None => return -9 };
    if msg_len > mq.msgsize { return -90; } // EMSGSIZE
    if mq.msgs.len() >= mq.maxmsg { return -11; } // EAGAIN
    let mut data = alloc::vec![0u8; msg_len];
    if copy_from_user(&mut data, buf_va).is_err() { return -14; }
    let pos = mq.msgs.partition_point(|m| m.priority >= msg_prio);
    mq.msgs.insert(pos, PosixMsg { priority: msg_prio, data });
    0
}
...
pub fn sys_mq_receive(mqdes: i32, buf_va: usize, msg_len: usize, prio_va: usize) -> isize {
    let mut tbl = POSIX_MQ.lock();
    let mq = match tbl.by_mqd.get_mut(&mqdes) { Some(m) => m, None => return -9 };
    if msg_len < mq.msgsize { return -90; }
    if mq.msgs.is_empty() { return -11; } // EAGAIN
    let msg = mq.msgs.remove(0);
    let copy_len = msg.data.len().min(msg_len);
    if copy_to_user(buf_va, &msg.data[..copy_len]).is_err() { return -14; }
    if prio_va != 0 {
        if copy_to_user(prio_va, &msg.priority.to_le_bytes()).is_err() { return -14; }
    }
    copy_len as isize
}
...
pub fn sys_mq_close(mqdes: i32) -> isize {
    ...
    if POSIX_MQ.lock().by_mqd.contains_key(&mqdes) { 0 } else { -9 }
}
...
pub fn drop_ipc_ns(ns: NsId) {
    ...
    let dead_mqds: Vec<i32> = pmq.by_name.iter()
        .filter(|((n, _), _)| *n == ns)
        .map(|(_, &mqd)| mqd)
        .collect();
    pmq.by_name.retain(|(n, _), _| *n != ns);
    for mqd in dead_mqds { pmq.by_mqd.remove(&mqd); }
}
