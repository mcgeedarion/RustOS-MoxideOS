...
//! ## Enforcement status
//!
//!   RLIMIT_CPU     (0)  — CPU time in seconds          ENFORCED  (interrupts: SIGXCPU/SIGKILL)
//!   RLIMIT_FSIZE   (1)  — max file size                ENFORCED  (write/writev/pwrite64: SIGXFSZ/EFBIG)
//!   RLIMIT_DATA    (2)  — max data segment size        ENFORCED  (sys_brk: ENOMEM)
//!   RLIMIT_STACK   (3)  — max stack size               ENFORCED  (exec/mmap)
//!   RLIMIT_CORE    (4)  — max core file size           ENFORCED  (mm/core_dump.rs: write gated; 0 = no dump)
//!   RLIMIT_RSS     (5)  — max resident set size        ENFORCED  (mm/rss.rs: charge on alloc; hard limit → ENOMEM)
//!   RLIMIT_NPROC   (6)  — max number of processes      ENFORCED  (fork)
//!   RLIMIT_NOFILE  (7)  — max open file descriptors    ENFORCED  (fcntl)
//!   RLIMIT_MEMLOCK (8)  — max locked memory bytes      ENFORCED  (mm/mlock.rs: ENOMEM on mlock/mlockall)
//!   RLIMIT_AS      (9)  — max virtual address space    ENFORCED  (mmap/brk)
//!   RLIMIT_LOCKS   (10) — max file locks               ENFORCED  (fs/flock.rs: ENOLCK)
//!   RLIMIT_SIGPENDING (11) — max pending signals       ENFORCED  (kill/tgkill/tkill)
//!   RLIMIT_MSGQUEUE   (12) — max POSIX MQ bytes        ENFORCED  (ipc/mq.rs: ENOMEM on mq_open/mq_send)
//!   RLIMIT_NICE    (13) — nice ceiling                 ENFORCED  (syscall/sched.rs: clamped on setpriority/sched_setattr)
//!   RLIMIT_RTPRIO  (14) — RT priority ceiling          ENFORCED  (syscall/sched.rs: EPERM if exceeded)
//!   RLIMIT_RTTIME  (15) — max RT CPU time (µs)         ENFORCED  (interrupts/scheduler: SIGXCPU/SIGKILL;
