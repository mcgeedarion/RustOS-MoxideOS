...
pub fn send_signal_group(tgid: usize, sig: i32) -> isize {
    ...
    send_signal_group_info(tgid, SigInfo {
        sig: sig as u32, code: SI_KERNEL, ..Default::default()
    });
    0
}
...
static SIGACTIONS: Mutex<BTreeMap<(usize, u32), SigAction>> =
    Mutex::new(BTreeMap::new());

pub fn get_sigaction(pid: usize, sig: u32) -> SigAction {
    SIGACTIONS.lock().get(&(pid, sig)).copied().unwrap_or_default()
}
...
struct SigFrameX86 {
    ...
    rip: u64, eflags: u64, cs: u16, _pad: [u16; 3],
    ss: u16, _pad2: [u16; 3],
    // siginfo
    si_signo: u32, si_errno: u32, si_code: i32,
    si_addr: u64,
    // sigmask
    sig_mask: u64,
}
...
fn push_sigframe_x86(
    ...
    if user_rsp < 0x1000 || user_rsp >= USER_SPACE_END { return false; }
    let sf = SigFrameX86 {
        pretcode: sa.restorer as u64,
        uc_flags: 0, uc_link: 0,
        uc_stack_ss_sp: 0, uc_stack_ss_flags: SS_DISABLE as u32, uc_stack_ss_size: 0,
        r8: 0, r9: 0, r10: 0, r11: frame.rflags as u64,
        r12: frame.r12 as u64, r13: frame.r13 as u64,
        r14: frame.r14 as u64, r15: frame.r15 as u64,
        rdi: frame.rdi as u64, rsi: frame.rsi as u64,
        rbp: frame.rbp as u64, rbx: frame.rbx as u64,
        rdx: frame.rdx as u64, rax: frame.rax as u64,
        rcx: frame.rip as u64, rsp: frame.rsp as u64,
        rip: frame.rip as u64, eflags: frame.rflags as u64,
        cs: 0x1b, _pad: [0; 3], ss: 0x23, _pad2: [0; 3],
        si_signo: info.sig, si_errno: 0, si_code: info.code,
...
fn push_sigframe_aarch64(
    ...
    if sp < 0x1000 || sp >= USER_SPACE_END { return false; }
...
fn apply_default(pid: usize, info: &SigInfo) {
    ...
    if SIG_IGN_DEFAULT & bit != 0 { return; }
    if SIG_STOP_DEFAULT & bit != 0 {
        scheduler::with_proc_mut(pid, |p| { p.state = State::Stopped; });
        return;
    }
    // Default terminate.
...
pub fn check_and_deliver(frame: &mut crate::arch::x86_64::syscall::SyscallFrame) {
    ...
    let info = pop_pending(tid)
        .or_else(|| pop_group_pending(tgid));

    let info = match info {
        Some(i) if !is_masked(tid, i.sig) => i,
        Some(i) => { put_back_pending(tid, i); return; }
        None => return,
    };
    ...
    if sa.handler == 1 { return; } // SIG_IGN
...
pub fn sys_rt_sigreturn(frame: &mut crate::arch::x86_64::syscall::SyscallFrame) {
    ...
    if !copy_from_user(sf_va, &mut bytes) { return; }
...
pub fn check_and_deliver_aarch64(
    frame: &mut crate::arch::aarch64::interrupts::ExceptionFrame,
) {
    let tid = scheduler::current_pid();
    let tgid = scheduler::with_proc(tid, |p| p.tgid).unwrap_or(tid);
...
pub fn sys_rt_sigprocmask(how: u32, set: Option<u64>, oldset: Option<&mut u64>, pid: usize) -> isize {
    let mut mask = SIGMASK.lock();
    let current = mask.entry(pid).or_insert(0);
    if let Some(old) = oldset { *old = *current; }
    if let Some(s) = set {
        match how {
            SIG_BLOCK   => *current |= s,
...
pub fn sys_rt_sigpending(pid: usize) -> u64 {
    let thread = PENDING.lock().get(&pid).map_or(0u64, |q| {
        q.iter().fold(0u64, |acc, i| acc | (1u64 << i.sig.saturating_sub(1)))
    });
...
pub fn send_sigchld(parent_pid: usize, child_pid: usize, exit_code: i32) {
    send_signal_group_info(parent_pid, SigInfo {
        sig:    17,
