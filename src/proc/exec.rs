...
use crate::mm::mmap::{Vma, VmaKind, MAP_ANON, MAP_GROWSDOWN, PAGE, PROT_READ, PROT_WRITE};
...
fn stack_bytes_for_pid(pid: usize) -> usize {
    let soft =
        scheduler::with_proc(pid, |p| p.rlimits.stack_soft()).unwrap_or(DEFAULT_STACK_BYTES as u64);
...
pub fn spawn_user_process(path: &str, argv: &[&str], envp: &[&str]) -> bool {
    ...
    if file_size == 0 || file_size > MAX_ELF {
        vfs::close(fd);
        return false;
    }
...
pub fn spawn_user_process_from_bytes(
    ...
    let program_entry = match elf::load_elf_into(new_cr3, data, &hdr, &phdrs) {
        ...
        Err(_) => {
            unsafe {
                free_old_address_space(new_cr3);
            }
            return false;
        }
    };

    let elf_bias = if hdr.e_type == elf::ET_DYN {
        ELF_DYN_BIAS
    } else {
        0
    };
    ...
    let (entry_va, interp_base_val) = if let Some(interp_path) = elf::find_interp(data, &phdrs) {
...
pub fn do_execve(
    ...
    let program_entry = elf::load_elf_into(new_cr3, data, &hdr, &phdrs).map_err(|e| {
        unsafe {
            free_old_address_space(new_cr3);
        }
        e
    ...
    let (entry_va, interp_base_val) = if let Some(interp_path) = elf::find_interp(data, &phdrs) {
        let e = load_interpreter(new_cr3, interp_path).map_err(|e| {
            unsafe {
                free_old_address_space(new_cr3);
            }
            e
    ...
    mmap::alloc_user_stack(new_cr3, STACK_TOP, stack_sz).map_err(|e| {
        unsafe {
            free_old_address_space(new_cr3);
        }
        e
    ...
    if old_cr3 != 0 {
        unsafe {
            free_old_address_space(old_cr3);
        }
    }
    load_cr3(new_cr3);
    ...
    let old_handlers =
        scheduler::with_proc(pid as usize, |p| p.signal_handlers.clone()).unwrap_or_default();
...
fn do_execve_riscv(path: &str, argv: &[&str], envp: &[&str]) -> Result<(), isize> {
    ...
    let program_entry = elf::load_elf_into(new_satp, data, &hdr, &phdrs).map_err(|e| {
        unsafe {
            free_old_address_space(new_satp);
        }
        e
    })?;
    ...
    let (entry_va, interp_base_val) = if let Some(interp_path) = elf::find_interp(data, &phdrs) {
        let e = load_interpreter(new_satp, interp_path).map_err(|e| {
            unsafe {
                free_old_address_space(new_satp);
            }
            e
    ...
    let stack_sz = stack_bytes_for_pid(pid as usize);
    mmap::alloc_user_stack(new_satp, STACK_TOP, stack_sz).map_err(|e| {
        unsafe {
            free_old_address_space(new_satp);
        }
        e
    ...
    if old_satp != 0 {
        unsafe {
            free_old_address_space(old_satp);
        }
    }
    load_cr3(new_satp);
    ...
    let kstack_top = scheduler::with_proc(pid as usize, |p| p.kstack_top).unwrap_or(0);
    if kstack_top == 0 {
        return Err(-1);
    }
...
fn read_cstr_array(ptr_array_va: usize) -> Vec<String> {
    ...
    loop {
        ...
        if copy_from_user(&mut ptr_buf, ptr_va).is_err() {
            break;
        }
...
fn build_initial_stack(
    ...
    macro_rules! push_bytes {
        ($bytes:expr) => {{
            ...
            sp &= !0xf;
            unsafe {
                ptr::copy_nonoverlapping(b.as_ptr(), sp as *mut u8, b.len());
            }
        }};
    }
    ...
    unsafe {
        ptr::copy_nonoverlapping(rand_bytes.as_ptr(), sp as *mut u8, 16);
    }
