...
//!      signal_handlers: SIG_IGN dispositions survive; user VAs reset to
//!      SIG_DFL. pending signals and sigmask are cleared (they don't survive
//!      exec).
...
pub fn spawn_user_process(path: &str, argv: &[&str], envp: &[&str]) -> bool {
    ...
    if vfs::read(fd, &mut data) != size as isize {
        vfs::close(fd);
        return false;
    }
    vfs::close(fd);
...
#[cfg(target_arch = "x86_64")]
pub fn do_execve(pid: usize, path_va: usize, argv_va: usize, envp_va: usize) -> isize {
    let path = match copy_cstr_from_user(path_va) {
        Some(s) => s,
        None => return -14,
    };
    ...
    scheduler::with_proc_mut(pid, |p, _pl| {
        mmap::clear_vmas(p);
        unsafe {
            paging::free_user_page_table(p.user_satp);
        }
        unsafe {
...
#[cfg(target_arch = "x86_64")]
fn do_execve_from_vecs(pid: usize, path: &str, argv: &[&str], envp: &[&str]) -> isize {
    let fd = match vfs::open(path, 0, 0) {
        fd if fd >= 0 => fd as usize,
        e => return e,
    };
...
#[cfg(target_arch = "riscv64")]
pub fn do_execve_riscv(pid: usize, path_va: usize, argv_va: usize, envp_va: usize) -> isize {
...
#[cfg(target_arch = "riscv64")]
fn do_execve_riscv_from_vecs(pid: usize, path: &str, argv: &[&str], envp: &[&str]) -> isize {
    use crate::arch::riscv64::paging::alloc_root_page_table;
    use crate::arch::riscv64::trap::{rebuild_trap_frame_riscv, TRAP_FRAME_SIZE};
    use crate::arch::riscv64::uentry::alloc_user_stack;
...
fn copy_strvec_from_user(vec_va: usize) -> Vec<String> {
    ...
    loop {
        let mut slot = [0u8; 8];
        if copy_from_user(ptr_va, &mut slot).is_err() {
            break;
        }
        ...
        if let Some(s) = copy_cstr_from_user(p) {
            out.push(s);
        }
        ptr_va += 8;
    }
