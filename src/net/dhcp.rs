...
fn build_packet(xid: u32, msg_type: u8, server_id: Option<u32>, requested: Option<u32>) -> Vec<u8> {
...
pub fn init() {
    ...
    let xid: u32 = 0xC0DE_0001u32.wrapping_add(crate::time::monotonic_ms() as u32);

    for attempt in 0..8u32 {
        *PENDING.lock() = None;
        let disc = build_packet(xid.wrapping_add(attempt), DHCP_DISCOVER, None, None);
        send_dhcp(&disc);
        ...
        *PENDING.lock() = None;
        ...
        if let Some(ack) = ack {
            ...
            *LEASE.lock() = Some(lease);
            ...
            ip::set_ip(lease.ip);
            if lease.gateway != 0 {
                ip::set_gw(lease.gateway);
            }
            if lease.mask != 0 {
                ip::set_mask(lease.mask);
            }
    ...
    while i < opts.len() {
        ...
        match tag {
            ...
            OPT_SERVERID if len >= 4 => {
                server = u32::from_be_bytes([val[0], val[1], val[2], val[3]])
            },
            OPT_SUBNET if len >= 4 => mask = u32::from_be_bytes([val[0], val[1], val[2], val[3]]),
            OPT_ROUTER if len >= 4 => {
                gateway = u32::from_be_bytes([val[0], val[1], val[2], val[3]])
            },
