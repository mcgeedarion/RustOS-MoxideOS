...
pub fn send_neighbor_solicitation(src: &Addr6, target: &Addr6) {
    ...
    body.push(OPT_SLLA); body.push(1);
...
fn handle_ns(src: &Addr6, dst: &Addr6, payload: &[u8]) {
    ...
    if target != our { return; } // not for us
...
fn send_neighbor_advertisement(dst: &Addr6, target: &Addr6) {
    ...
    body.push(OPT_TLLA); body.push(1);
    body.extend_from_slice(&our_mac);
        ...
        match opt_type {
            ...
            OPT_PIO => {
                ...
                if opt_off + opt_len >= 32 {
                    ...
                    let prefix: Addr6 = payload[opt_off+16..opt_off+32].try_into().unwrap_or(UNSPECIFIED6);
                    if a_bit {
...
pub fn receive(src: &Addr6, dst: &Addr6, payload: &[u8]) {
    ...
    // Verify checksum.
    if checksum(src, dst, payload) != 0 { return; }
