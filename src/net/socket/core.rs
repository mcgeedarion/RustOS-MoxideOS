...
use super::types::*;
use super::address::{read_sockaddr_in, read_sockaddr_in6, write_sockaddr_in,
                     write_sockaddr_in6, next_ephemeral};
use super::buffer::{UnixConn, UnixListener};
use super::unix::stream::{unix_bind, unix_listen, unix_accept, unix_connect};
...
pub fn new_socket(domain: u16, kind: u16, protocol: u32) -> Socket {
    Socket {
        ...
        local: None, peer: None,
        tcp_id: None,
        udp_rx: alloc::collections::VecDeque::new(),
        nonblock: false, reuse_addr: false, keepalive: false,
        unix_conn: None, unix_listener: None,
        so_error: 0,
    }
}
...
pub fn sys_bind(fd: usize, addr_va: usize, _addrlen: u32) -> isize {
    let domain = {
        let s = SOCKETS.lock();
        s.get(fd).and_then(|x| x.as_ref()).map(|x| x.domain).unwrap_or(0)
    };
    if domain == AF_UNIX {
    ...
    if domain == AF_INET6 {
        let Some((port, ip6, _, _)) = super::address::read_sockaddr_in6(addr_va) else { return -22; };
        let port = if port == 0 { next_ephemeral() } else { port };
        let mut sockets = SOCKETS.lock();
        if let Some(Some(s)) = sockets.get_mut(fd) {
            s.local = Some(SockAddr::V6 { ip: ip6, port, flowinfo: 0, scope_id: 0 });
            s.state = SocketState::Bound;
        }
        return 0;
    }
    let Some((port, ip)) = read_sockaddr_in(addr_va) else { return -22; };
    let port = if port == 0 { next_ephemeral() } else { port };
    let mut sockets = SOCKETS.lock();
    if let Some(Some(s)) = sockets.get_mut(fd) {
...
pub fn sys_listen(fd: usize, backlog: i32) -> isize {
    ...
    if domain == AF_UNIX { return unix_listen(fd, backlog as usize); }
    let local_port = {
        let s = SOCKETS.lock();
        match s.get(fd).and_then(|x| x.as_ref()).and_then(|x| x.local.as_ref()) {
    ...
    let mut sockets = SOCKETS.lock();
    if let Some(Some(s)) = sockets.get_mut(fd) { s.state = SocketState::Listening; }
    0
}
...
pub fn sys_accept(fd: usize, addr_va: usize, addrlen_va: usize) -> isize {
    ...
    for (i, slot) in sockets.iter_mut().enumerate() {
        if slot.is_none() {
            ...
            s.peer   = Some(SockAddr::V4 { ip: peer_ip, port: peer_port });
            *slot = Some(s);
            return i as isize;
        }
    }
...
pub fn sys_connect(fd: usize, addr_va: usize, _addrlen: u32) -> isize {
    ...
    let peer = if domain == AF_INET6 {
        let Some((port, ip6, flowinfo, scope_id)) =
            super::address::read_sockaddr_in6(addr_va) else { return -22; };
        SockAddr::V6 { ip: ip6, port, flowinfo, scope_id }
    } else {
        let Some((port, ip)) = read_sockaddr_in(addr_va) else { return -22; };
        SockAddr::V4 { ip, port }
    };
...
pub fn sys_getsockname(fd: usize, addr_va: usize, addrlen_va: usize) -> isize {
    ...
    match &s.local {
        Some(SockAddr::V4 { ip, port }) => {
            ...
            if addrlen_va != 0 { let sz: u32 = 16; copy_to_user(addrlen_va, &sz.to_ne_bytes()); }
        }
        Some(SockAddr::V6 { ip, port, flowinfo, scope_id }) => {
            drop(sockets);
            write_sockaddr_in6(addr_va, ip, *port, *flowinfo, *scope_id);
            if addrlen_va != 0 { let sz: u32 = 28; copy_to_user(addrlen_va, &sz.to_ne_bytes()); }
        }
        _ => {}
    }
    0
