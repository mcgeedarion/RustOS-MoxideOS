...
use super::types::{AF_UNIX, SOCK_STREAM, SocketState, MSG_DONTWAIT};
...
pub fn sys_socketpair(domain: u16, kind: u16, _proto: u32, sv_va: usize) -> isize {
    if domain != AF_UNIX || kind != SOCK_STREAM { return -22; }
    let (ca, cb) = UnixConn::new_pair();
    let (fa, fb) = (alloc_socket_pair_slot(ca), alloc_socket_pair_slot(cb));
    if fa < 0 || fb < 0 { return -24; }
    let pair = [(fa as usize) as u32, (fb as usize) as u32];
    copy_to_user(sv_va, unsafe { core::slice::from_raw_parts(pair.as_ptr() as *const u8, 8) });
    0
}
...
pub fn socket_close(fd: usize) {
    ...
    if let Some(Some(sock)) = sockets.get_mut(fd) {
        if let Some(conn) = sock.unix_conn.take() { conn.close_tx(); }
        if let Some(id) = sock.tcp_id.take() { crate::net::tcp::close(id); }
    }
    if let Some(slot) = sockets.get_mut(fd) { *slot = None; }
}