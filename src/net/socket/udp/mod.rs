...
pub fn sys_sendto(
    fd: usize, buf_va: usize, len: usize, _flags: u32,
    dest_va: usize, _dest_len: u32,
) -> isize {
    ...
    let src_port = {
        let sockets = SOCKETS.lock();
        match sockets.get(fd).and_then(|s| s.as_ref()).and_then(|s| s.local.as_ref()) {
            Some(SockAddr::V4 { port, .. }) => *port,
            _ => next_ephemeral(),
        }
    };
...
pub fn sys_recvfrom(
    fd: usize, buf_va: usize, len: usize, _flags: u32,
    src_addr_va: usize, src_len_va: usize,
) -> isize {
    let entry = {
        let mut sockets = SOCKETS.lock();
        let Some(Some(s)) = sockets.get_mut(fd) else { return -9; };
        s.udp_rx.pop_front()
    };
    let Some((src_ip, src_port, data)) = entry else { return -11; };
    let n = data.len().min(len);
    let mut out = alloc::vec![0u8; n];
    out.copy_from_slice(&data[..n]);
    copy_to_user(buf_va, &out);
    if src_addr_va != 0 { write_sockaddr_in(src_addr_va, src_ip, src_port); }
    if src_len_va  != 0 {
        let sz: u32 = 16;
