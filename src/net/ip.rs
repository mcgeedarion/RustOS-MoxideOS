...
/// Our IPv4 address (network byte order, stored as host u32).
static OUR_IP:      Mutex<u32> = Mutex::new(0x0A00_0202); // 10.0.2.2 (qemu user-mode default)
static GATEWAY_IP:  Mutex<u32> = Mutex::new(0x0A00_0202); // same as above until DHCP
...
pub fn send(dst_ip: u32, proto: u8, payload: &[u8]) {
    ...
    hdr[4]  = (id >> 8) as u8;
    hdr[5]  = id as u8;
    hdr[6]  = 0;    // flags=0 (DF not set), frag offset=0
    hdr[7]  = 0;
    hdr[8]  = 64;   // TTL
    hdr[9]  = proto;
    // checksum at [10..12] = 0 for now
    ...
    // Route: same subnet → direct ARP; otherwise → gateway
    ...
    if let Some(mac) = arp::resolve(next_hop) {
        eth::send(mac, ETHERTYPE_IP, &pkt);
    }
    // If ARP misses, packet is dropped; upper layer must retry.
}
