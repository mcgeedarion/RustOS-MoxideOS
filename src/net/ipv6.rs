...
pub fn solicited_node(addr: &Addr6) -> Addr6 {
    ...
    m[0]  = 0xFF; m[1] = 0x02;
    m[11] = 0x01; m[12] = 0xFF;
    m[13] = addr[13]; m[14] = addr[14]; m[15] = addr[15];
    m
}
...
pub fn is_multicast(a: &Addr6) -> bool { a[0] == 0xFF }
pub fn is_link_local(a: &Addr6) -> bool { a[0] == 0xFE && (a[1] & 0xC0) == 0x80 }
pub fn is_loopback(a: &Addr6) -> bool { *a == LOOPBACK6 }
...
pub fn our_ip6()                  -> Addr6 { *OUR_IP6.lock() }
pub fn set_ip6(a: Addr6)                   { *OUR_IP6.lock()    = a; }
pub fn set_gateway6(a: Addr6)              { *GATEWAY6.lock()   = a; }
pub fn set_prefix_len(p: u8)               { *PREFIX_LEN.lock() = p; }
...
/// Insert a fragment and attempt to reassemble.  Returns Some(data) when complete.
fn frag_insert(src: Addr6, dst: Addr6, id: u32, nh: u8,
               offset: u16, last: bool, data: &[u8]) -> Option<(u8, Vec<u8>)>
{
    ...
    let idx = match pos {
        Some(i) => i,
        None => {
            if table.len() >= MAX_FRAG_BUFS { table.remove(0); }
            table.push(FragBuf {
                src, dst, id, nh,
                frags: Vec::new(),
                created: now,
    ...
    // Check if we have all fragments: offset 0 present, one fragment with last=true,
    // and no gaps.
    let has_last = table[idx].frags.iter().any(|(_, l, _)| *l);
    let has_first = table[idx].frags.first().map(|(o,_,_)| *o == 0).unwrap_or(false);
    if !has_last || !has_first { return None; }

    let mut expected = 0u16;
    let mut ok = true;
    for (off, _, d) in &table[idx].frags {
        if *off != expected { ok = false; break; }
    ...
    let mut out = Vec::new();
    for (_, _, d) in &table[idx].frags { out.extend_from_slice(d); }
    table.remove(idx);
    Some((nh_out, out))
}
...
fn walk_ext_headers(pkt: &[u8], mut off: usize, mut nh: u8)
    -> Option<(u8, usize)>
{
    loop {
        match nh {
            NH_HOPOPT | NH_ROUTING | NH_DSTOPT => {
                ...
                if off + hlen > pkt.len() { return None; }
                nh  = next;
                off += hlen;
            }
            NH_FRAG => {
                ...
                if pkt.len() < IPV6_HDR_LEN { return None; }
                let src: Addr6 = pkt[8..24].try_into().ok()?;
                let dst: Addr6 = pkt[24..40].try_into().ok()?;
                if let Some((rnh, data)) = frag_insert(src, dst, id, next_nh,
                                                        frag_off, !more_frags, &payload) {
                    ...
                    receive6_upper(rnh, &pkt[8..24].try_into().ok()?,
...
pub fn receive6(frame: &[u8]) {
    if frame.len() < IPV6_HDR_LEN { return; }
    ...
    if frame.len() < IPV6_HDR_LEN + payload_len { return; }
...
/// Fragmentation of large payloads is not implemented here — the caller must
/// ensure `payload` fits within the path MTU (typically 1280 bytes for link-local).
pub fn send6(dst: &Addr6, next_header: u8, payload: &[u8]) {
    ...
    let next_hop_mac = if is_multicast(dst) {
        ...
        if same_prefix {
            ...
            if gw == UNSPECIFIED6 { return; } // no gateway
