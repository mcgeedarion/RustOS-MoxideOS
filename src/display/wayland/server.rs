...
//!   3. init forks and execs `/usr/bin/rustos-compositor`, passing the open fds
//!      via the `WAYLAND_DRM_FD` and `WAYLAND_INPUT_FD` environment variables.
//!   4. init sits in a `waitpid(-1, ...)` supervisor loop.  If the compositor
//!      crashes or exits, init closes the old device fds, sleeps one second,
//!      reopens them (releasing and reacquiring DRM master), and re-execs the
//!      compositor binary.
//!
...
//!   - A compositor crash produces an ordinary `SIGCHLD` to init, not a kernel
//!     panic.
//!   - The compositor binary is restricted by a seccomp filter to fewer than 15
//!     syscalls; restart policy is enforced without kernel changes.
...
//!   3. Support `AF_UNIX` sockets so the compositor can bind `/run/wayland-0`
//!      and accept client connections.
//!   4. Deliver vblank events via `drm::deliver_vblank_event()`, called from
//!      the DRM vblank ISR through `compositor::vblank_notify()`.
//!
